#ifndef CHANGERENDERTARGETACTION_H
#define CHANGERENDERTARGETACTION_H
...
class ChangeRenderTargetAction: public AbstractAction
{
	friend class Trigger;
	friend class AbstractAction;
	private: StringANSI argName;
    private: StringExprParserEx templateArg;
    private: MagicPointer<Texture2D> arg;

	private: StringANSI targetName;
	private: StringExprParserEx templateTarget;
	private: MagicPointer<GraphicsContainer> target;
	private: ChangeRenderTargetAction(void);
	private: ChangeRenderTargetAction(int32 _type);
	public: virtual ~ChangeRenderTargetAction(void);
