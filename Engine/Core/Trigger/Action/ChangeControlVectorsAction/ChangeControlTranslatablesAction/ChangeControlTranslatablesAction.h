#ifndef CHANGECONTROLTRANSLATABLESACTION_H
#define CHANGECONTROLTRANSLATABLESACTION_H
...
class ChangeControlTranslatablesAction: public AbstractAction
{
	friend class Trigger;
	friend class AbstractAction;
	private: StringANSI argName;
    private: StringExprParserEx templateArg;
    private: MagicPointer<List<TranslatableObject>> arg;

	private: StringANSI targetName;
	private: StringExprParserEx templateTarget;
	private: MagicPointer<VectorLerp> target;
	private: ChangeControlTranslatablesAction(void);
	private: ChangeControlTranslatablesAction(int32 _type);
	public: virtual ~ChangeControlTranslatablesAction(void);
