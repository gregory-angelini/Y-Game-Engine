#ifndef CHANGECONTROLVECTORSACTION_H
#define CHANGECONTROLVECTORSACTION_H
...
class ChangeControlVectorsAction: public AbstractAction
{
	friend class Trigger;
	friend class AbstractAction;
	private: StringANSI argName;
    private: StringExprParserEx templateArg;
    private: MagicPointer<List<Vector3>> arg;

	private: StringANSI targetName;
	private: StringExprParserEx templateTarget;
	private: MagicPointer<VectorLerp> target;
	private: ChangeControlVectorsAction(void);
	private: ChangeControlVectorsAction(int32 _type);
	public: virtual ~ChangeControlVectorsAction(void);
