#ifndef CHANGECONTROLSCALABLESACTION_H
#define CHANGECONTROLSCALABLESACTION_H
...
class ChangeControlScalablesAction: public AbstractAction
{
	friend class Trigger;
	friend class AbstractAction;
	private: StringANSI argName;
    private: StringExprParserEx templateArg;
    private: MagicPointer<List<ScalableObject>> arg;

	private: StringANSI targetName;
	private: StringExprParserEx templateTarget;
	private: MagicPointer<VectorLerp> target;
	private: ChangeControlScalablesAction(void);
	private: ChangeControlScalablesAction(int32 _type);
	public: virtual ~ChangeControlScalablesAction(void);
