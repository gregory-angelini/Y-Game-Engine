#ifndef CHANGECONTROLROTATABLESACTION_H
#define CHANGECONTROLROTATABLESACTION_H
...
class ChangeControlRotatablesAction: public AbstractAction
{
	friend class Trigger;
	friend class AbstractAction;
	private: StringANSI argName;
    private: StringExprParserEx templateArg;
    private: MagicPointer<List<RotatableObject>> arg;

	private: StringANSI targetName;
	private: StringExprParserEx templateTarget;
	private: MagicPointer<QuaternionLerp> target;
	private: ChangeControlRotatablesAction(void);
	private: ChangeControlRotatablesAction(int32 _type);
	public: virtual ~ChangeControlRotatablesAction(void);
