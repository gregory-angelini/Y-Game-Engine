...
class Application: public Renderer
{
	...
	private: Quad2D* quad1, *quad2;// трехмерный графический квад
	private: Texture2D *texture;// указатель на текстуру
	...
	public: bool Create(Renderer::Define _define)
	{
		...
		if(Renderer::Create(_define))
		{
			...
			// текстура...
			{
				...
				scene->AddObject(texture);// добавить в сцену	
			}
			...
			// полигональная сетка...
			{
				...
				quad1->SetPosition(Vector3D(1,0,-1.5));// отодвинуть квад вглубь экрана по оси Z
				quad1->Rename("quad1");

				// текстурный материал
				quad1->BindTexture(texture);// привязать текстуру texture к полигональной сетке
				//quad1->ComputeTexelsForQuadShape();// вычислить тексели для квада
				quad1->SetEnable(true);// включить материал
		
				scene->AddObject(quad1);// добавить в сцену	
			...
			// полигональная сетка...
			{
				...
				quad2->SetPosition(Vector3D(-1,0,-2));// отодвинуть квад вглубь экрана по оси Z
				quad2->Rename("quad2");

				// текстурный материал
				quad2->BindTexture(texture);// привязать текстуру texture к полигональной сетке
				//quad2->ComputeTexelsForQuadShape();// вычислить тексели для квада
				quad2->SetEnable(true);// включить материал
	
				
				scene->AddObject(quad2);// добавить в сцену	 
	...
	public: virtual bool KeyboardEventFilter(Keyboard::Event _event)
	{
		if(_event.type == Keyboard::KEY_DOWN)
		{
			...
			{
				// создаем текстуру, и заносим в нее изображение клиентской области окна
				Texture2D txr;
				txr.Create(ClientAreaSize());
				txr.FromScreen(Vector2D(), ClientAreaSize());

				// записываем изображение в .png файл
				PixelImage image;
				txr.GetImage(image);
