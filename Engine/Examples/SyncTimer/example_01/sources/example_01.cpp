...
class Application: public Engine
{
	...
	private: SyncTimer timer;// синхронный таймер
	...
	public: StringANSI NameOfMode(SyncTimer::Mode _mode)
	{
		switch(_mode)
		{
			case SyncTimer::SINGLE_SHOT:
			{
				return "SINGLE_SHOT";
			}
			case SyncTimer::EVER_SHOT:
			{
				return "EVER_SHOT";
			}
			case SyncTimer::INTERVAL:
			{
				return "INTERVAL";
			}
		}
	...
	public: virtual void Draw(float _delta)
	{
		...
		font2d->DrawScreenText(Vector3D(10, ClientAreaSize().height - (i+=20), -1), "timer.Check: %d", timer.Check(_delta)); 
	...
	public: virtual bool KeyboardEventFilter(Keyboard::Event _event)
	{
		if(_event.type == Keyboard::KEY_DOWN)
		{
			...
			{
				timer.SetMode(SyncTimer::EVER_SHOT);
			}
			else if(_event.key == Keyboard::NUM_3)
			{
				timer.SetMode(SyncTimer::INTERVAL);
			}
			...
			{
				SyncTimer::Define def(4, 1, SyncTimer::INTERVAL, false);
				timer.Create(def);
			}
				   ...
				   int showCmd)
{
	...
	def.rendererDef.realTimeWorkerDef.windowDef.tittle = "SyncTimer.example_01";
