         	       ...
         	       public MessageHandler<int32>::Receiver// получатель сообщений
{
	...
	private: GraphicsPointEx* currentVertex;// текущий узел интерполяции
	...
	public: bool Create(Engine::Define _define)
	{
		...
		if(Engine::Create(_define))
		{
			...
			{// интерполятор векторов...
				vectorLerp = new VectorLerp();
				vectorLerp->SetAmountOfSegment(20);

				vectorLerp->SetMoveSpeed(1.0f);// скорость движения 
				vectorLerp->SetTarget(5.0f);// расстояние до цели
				vectorLerp->SetMovingMode(FloatLerp::DAMPING);

				vectorLerp->SetFreezeEnable(true);// заморозить
				scene->AddLogicObject(vectorLerp);
			}


			// курсор "разсжатая рука"...
			{
				...
				unclenchedHandCursor->SetShowEnable(true);// выключить отображени
			}
			...
			// курсор "сжатая рука"...
			{
				...
				clenchedHandCursor->SetShowEnable(true);// выключить отображение
			}
			...
			// текущая вершина...
			{
				...
				currentVertex->SetColor(RGBA(0,255,255,255));// цвет вершины
				currentVertex->SetShowEnable(false);// выключить отображение
				scene->AddGraphicsObject(currentVertex);// добавить в графическую сцену
	...
	public: virtual void Update(float _delta)
	{
		...
		if(vectorLerp->IsMovingEnabled() && !currentVertex->IsShowEnabled())// выполнить один раз
		{
			currentVertex->SetShowEnable(true);// отобразить
		}
		else if(!vectorLerp->IsMovingEnabled())// если цель достигнута
		{
			if(currentVertex->IsShowEnabled())// выполнить один раз
			{
				currentVertex->SetShowEnable(false);// скрыть
			}
		}
	}
	...
	public: virtual Mouse::Event* MouseEventFilter(Mouse::Event* _event)
	{
		...
		{
			if(lockedVertex)// если перемещаем вершину
			{
				// обновить кривую... 
