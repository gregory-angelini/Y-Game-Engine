...
void ActionTableWidget::COPY_TILE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "COPY_TILE")
	{
...
bool ActionTableWidget::COPY_TILE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "COPY_TILE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Source tile"); 
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "Target tile"); 
		...
		{ 
			...
			SetCellText(ARG3_PROPERTY, CONTENT, "Copy result"); 
...
QString ActionTableWidget::COPY_TILE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "COPY_TILE")
	{ 
		QString name;
...
bool ActionTableWidget::COPY_TILE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "COPY_TILE")
	{ 
...
bool ActionTableWidget::COPY_TILE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "COPY_TILE")
	{
...
bool ActionTableWidget::COPY_TILE_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "COPY_TILE")
	{ 
...
bool ActionTableWidget::COPY_TILE_UpdateArg3VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "COPY_TILE")
	{
...
void ActionTableWidget::COPY_TILE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "COPY_TILE")
	{
...
void ActionCreator::COPY_TILE_SetAction(int _actionType)
{
	if(_actionType == TiledSpace3D::COPY_TILE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "COPY_TILE");

		CopyTileAction* derived = dynamic_cast<CopyTileAction*>(action);
...
void ActionCreator::COPY_TILE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "COPY_TILE")
	{
		action = Trigger::_CreateAction(TiledSpace3D::COPY_TILE_MESSAGE);
