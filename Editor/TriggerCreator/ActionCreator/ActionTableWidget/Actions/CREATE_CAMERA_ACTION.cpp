...
void ActionTableWidget::CREATE_CAMERA_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_CAMERA")
	{
...
bool ActionTableWidget::CREATE_CAMERA_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_CAMERA") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Camera name"); 
...
QString ActionTableWidget::CREATE_CAMERA_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_CAMERA")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_CAMERA_UpdateArg1KeyPropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_CAMERA")
	{ 
...
bool ActionTableWidget::CREATE_CAMERA_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_CAMERA")
	{
...
void ActionTableWidget::CREATE_CAMERA_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_CAMERA")
	{
...
void ActionCreator::CREATE_CAMERA_SetAction(int _actionType)
{
	if(_actionType == Camera3D::CREATE_CAMERA_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_CAMERA");

		CreateCameraAction* derived = dynamic_cast<CreateCameraAction*>(action);
...
void ActionCreator::CREATE_CAMERA_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_CAMERA")
	{
		action = Trigger::_CreateAction(Camera3D::CREATE_CAMERA_MESSAGE);
