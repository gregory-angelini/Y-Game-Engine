...
void ActionTableWidget::CHANGE_TILED_SPACE_SIZE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_TILED_SPACE_SIZE")
	{
...
bool ActionTableWidget::CHANGE_TILED_SPACE_SIZE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_TILED_SPACE_SIZE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Space size"); 
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "Tile size"); 
...
QString ActionTableWidget::CHANGE_TILED_SPACE_SIZE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_TILED_SPACE_SIZE")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_TILED_SPACE_SIZE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_TILED_SPACE_SIZE")
	{ 
...
bool ActionTableWidget::CHANGE_TILED_SPACE_SIZE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_TILED_SPACE_SIZE")
	{
...
bool ActionTableWidget::CHANGE_TILED_SPACE_SIZE_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_TILED_SPACE_SIZE")
	{ 
...
void ActionTableWidget::CHANGE_TILED_SPACE_SIZE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_TILED_SPACE_SIZE")
	{
...
void ActionCreator::CHANGE_TILED_SPACE_SIZE_SetAction(int _actionType)
{
	if(_actionType == TiledSpace3D::CHANGE_TILED_SPACE_SIZE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_TILED_SPACE_SIZE");

		ChangeTiledSpaceSizeAction* derived = dynamic_cast<ChangeTiledSpaceSizeAction*>(action);
...
void ActionCreator::CHANGE_TILED_SPACE_SIZE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_TILED_SPACE_SIZE")
	{
		action = Trigger::_CreateAction(TiledSpace3D::CHANGE_TILED_SPACE_SIZE_MESSAGE);
