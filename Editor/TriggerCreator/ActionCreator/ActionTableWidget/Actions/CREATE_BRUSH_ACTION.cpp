...
void ActionTableWidget::CREATE_BRUSH_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_BRUSH")
	{
...
bool ActionTableWidget::CREATE_BRUSH_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_BRUSH") 
	{
		...
		{ 
			...
			SetCellText(ARG1_PROPERTY, CONTENT, "Sample"); 
			SetCellText(ARG1_PROPERTY, DATA_TYPE, "Texturing surface"); 
		}
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "Pass enable"); 
...
QString ActionTableWidget::CREATE_BRUSH_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_BRUSH")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_BRUSH_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_BRUSH")
	{ 
...
bool ActionTableWidget::CREATE_BRUSH_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_BRUSH")
	{
...
bool ActionTableWidget::CREATE_BRUSH_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_BRUSH")
	{ 
...
bool ActionTableWidget::CREATE_BRUSH_UpdateArg3VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_BRUSH")
	{ 
...
void ActionTableWidget::CREATE_BRUSH_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_BRUSH")
	{
...
void ActionCreator::CREATE_BRUSH_SetAction(int _actionType)
{
	if(_actionType == TiledSpace3D::CREATE_BRUSH_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_BRUSH");

		CreateBrushAction* derived = dynamic_cast<CreateBrushAction*>(action);
...
void ActionCreator::CREATE_BRUSH_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_BRUSH")
	{
		action = Trigger::_CreateAction(TiledSpace3D::CREATE_BRUSH_MESSAGE);
