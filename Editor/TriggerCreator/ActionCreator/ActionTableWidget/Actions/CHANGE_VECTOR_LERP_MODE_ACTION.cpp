...
void ActionTableWidget::CHANGE_VECTOR_LERP_MODE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_VECTOR_LERP_MODE")
	{
...
bool ActionTableWidget::CHANGE_VECTOR_LERP_MODE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_VECTOR_LERP_MODE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Vector lerp mode"); 
...
QString ActionTableWidget::CHANGE_VECTOR_LERP_MODE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_VECTOR_LERP_MODE")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_VECTOR_LERP_MODE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_VECTOR_LERP_MODE")
	{ 
...
bool ActionTableWidget::CHANGE_VECTOR_LERP_MODE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_VECTOR_LERP_MODE")
	{
...
void ActionTableWidget::CHANGE_VECTOR_LERP_MODE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_VECTOR_LERP_MODE")
	{
...
void ActionCreator::CHANGE_VECTOR_LERP_MODE_SetAction(int _actionType)
{
	if(_actionType == VectorLerp::CHANGE_VECTOR_LERP_MODE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_VECTOR_LERP_MODE");

		ChangeVectorLerpModeAction* derived = dynamic_cast<ChangeVectorLerpModeAction*>(action);
...
void ActionCreator::CHANGE_VECTOR_LERP_MODE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_VECTOR_LERP_MODE")
	{
		action = Trigger::_CreateAction(VectorLerp::CHANGE_VECTOR_LERP_MODE_MESSAGE);
