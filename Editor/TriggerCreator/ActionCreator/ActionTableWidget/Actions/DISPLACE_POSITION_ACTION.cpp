...
void ActionTableWidget::DISPLACE_POSITION_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "DISPLACE_POSITION")
	{
...
bool ActionTableWidget::DISPLACE_POSITION_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "DISPLACE_POSITION") 
	{
...
QString ActionTableWidget::DISPLACE_POSITION_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "DISPLACE_POSITION")
	{ 
		QString name;
...
bool ActionTableWidget::DISPLACE_POSITION_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "DISPLACE_POSITION")
	{ 
...
bool ActionTableWidget::DISPLACE_POSITION_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "DISPLACE_POSITION")
	{
...
void ActionTableWidget::DISPLACE_POSITION_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "DISPLACE_POSITION")
	{
...
void ActionCreator::DISPLACE_POSITION_SetAction(int _actionType)
{
	if(_actionType == TranslatableObject::DISPLACE_POSITION_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "DISPLACE_POSITION");

		DisplacePositionAction* derived = dynamic_cast<DisplacePositionAction*>(action);
...
void ActionCreator::DISPLACE_POSITION_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "DISPLACE_POSITION")
	{
		action = Trigger::_CreateAction(TranslatableObject::DISPLACE_POSITION_MESSAGE);
