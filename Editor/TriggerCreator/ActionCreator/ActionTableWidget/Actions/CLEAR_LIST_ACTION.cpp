...
void ActionTableWidget::CLEAR_LIST_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CLEAR_LIST")
	{
...
bool ActionTableWidget::CLEAR_LIST_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CLEAR_LIST") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Remove mode"); 
...
QString ActionTableWidget::CLEAR_LIST_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CLEAR_LIST")
	{ 
		QString name;
...
bool ActionTableWidget::CLEAR_LIST_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CLEAR_LIST")
	{ 
...
bool ActionTableWidget::CLEAR_LIST_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CLEAR_LIST")
	{
...
void ActionTableWidget::CLEAR_LIST_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CLEAR_LIST")
	{
...
void ActionCreator::CLEAR_LIST_SetAction(int _actionType)
{
	if(_actionType == AbstractList::CLEAR_LIST_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CLEAR_LIST");

		ClearListAction* derived = dynamic_cast<ClearListAction*>(action);
...
void ActionCreator::CLEAR_LIST_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CLEAR_LIST")
	{
		action = Trigger::_CreateAction(AbstractList::CLEAR_LIST_MESSAGE);
