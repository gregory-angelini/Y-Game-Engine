...
void ActionTableWidget::CHECK_LINE_IN_SURFACE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHECK_LINE_IN_SURFACE")
	{
...
bool ActionTableWidget::CHECK_LINE_IN_SURFACE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHECK_LINE_IN_SURFACE") 
	{
		...
		{ 
			...
			SetCellText(ARG5_PROPERTY, CONTENT, "Intersected polygon"); 
		...
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Abstract surface"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::CHECK_LINE_IN_SURFACE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_SURFACE")
	{ 
		QString name;
...
bool ActionTableWidget::CHECK_LINE_IN_SURFACE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_SURFACE")
	{ 
...
bool ActionTableWidget::CHECK_LINE_IN_SURFACE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_SURFACE")
	{
...
bool ActionTableWidget::CHECK_LINE_IN_SURFACE_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_SURFACE")
	{
...
bool ActionTableWidget::CHECK_LINE_IN_SURFACE_UpdateArg3VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_SURFACE")
	{
...
bool ActionTableWidget::CHECK_LINE_IN_SURFACE_UpdateArg4VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_SURFACE")
	{
...
bool ActionTableWidget::CHECK_LINE_IN_SURFACE_UpdateArg5VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_SURFACE")
	{
...
void ActionTableWidget::CHECK_LINE_IN_SURFACE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_SURFACE")
	{
...
void ActionCreator::CHECK_LINE_IN_SURFACE_SetAction(int _actionType)
{
	if(_actionType == AbstractSurface::CHECK_LINE_IN_SURFACE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHECK_LINE_IN_SURFACE");

		CheckLineInSurfaceAction* derived = dynamic_cast<CheckLineInSurfaceAction*>(action);
...
void ActionCreator::CHECK_LINE_IN_SURFACE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHECK_LINE_IN_SURFACE")
	{
		action = Trigger::_CreateAction(AbstractSurface::CHECK_LINE_IN_SURFACE_MESSAGE);
