...
void ActionTableWidget::CHANGE_COLLISION_GROUP_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_COLLISION_GROUP")
	{
...
bool ActionTableWidget::CHANGE_COLLISION_GROUP_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_COLLISION_GROUP") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Group index"); 
...
QString ActionTableWidget::CHANGE_COLLISION_GROUP_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_COLLISION_GROUP")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_COLLISION_GROUP_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_COLLISION_GROUP")
	{ 
...
bool ActionTableWidget::CHANGE_COLLISION_GROUP_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_COLLISION_GROUP")
	{
...
void ActionTableWidget::CHANGE_COLLISION_GROUP_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_COLLISION_GROUP")
	{
...
void ActionCreator::CHANGE_COLLISION_GROUP_SetAction(int _actionType)
{
	if(_actionType == PolygonalSurface::CHANGE_COLLISION_GROUP_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_COLLISION_GROUP");

		ChangeCollisionGroupAction* derived = dynamic_cast<ChangeCollisionGroupAction*>(action);
...
void ActionCreator::CHANGE_COLLISION_GROUP_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_COLLISION_GROUP")
	{
		action = Trigger::_CreateAction(PolygonalSurface::CHANGE_COLLISION_GROUP_MESSAGE);
