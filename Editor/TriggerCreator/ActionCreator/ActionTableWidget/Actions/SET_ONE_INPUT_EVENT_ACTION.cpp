...
void ActionTableWidget::SET_ONE_INPUT_EVENT_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "SET_ONE_INPUT_EVENT")
	{
...
bool ActionTableWidget::SET_ONE_INPUT_EVENT_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "SET_ONE_INPUT_EVENT") 
	{
		...
		else if(_index.row() == ARG1_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Input device"); 
		...
		{ 
			...
			if(item(ARG3_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG3_PROPERTY, CONTENT, "Event key"); 
...
QString ActionTableWidget::SET_ONE_INPUT_EVENT_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SET_ONE_INPUT_EVENT")
	{ 
		QString name;
...
bool ActionTableWidget::SET_ONE_INPUT_EVENT_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SET_ONE_INPUT_EVENT")
	{ 
...
bool ActionTableWidget::SET_ONE_INPUT_EVENT_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SET_ONE_INPUT_EVENT")
	{
...
bool ActionTableWidget::SET_ONE_INPUT_EVENT_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SET_ONE_INPUT_EVENT")
	{
...
bool ActionTableWidget::SET_ONE_INPUT_EVENT_UpdateArg3VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SET_ONE_INPUT_EVENT")
	{
...
void ActionTableWidget::SET_ONE_INPUT_EVENT_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SET_ONE_INPUT_EVENT")
	{
...
void ActionCreator::SET_ONE_INPUT_EVENT_SetAction(int _actionType)
{
	if(_actionType == PlayerControl::SET_ONE_INPUT_EVENT_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "SET_ONE_INPUT_EVENT");

		SetOneInputEventAction* derived = dynamic_cast<SetOneInputEventAction*>(action);
...
void ActionCreator::SET_ONE_INPUT_EVENT_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "SET_ONE_INPUT_EVENT")
	{
		action = Trigger::_CreateAction(PlayerControl::SET_ONE_INPUT_EVENT_MESSAGE);
