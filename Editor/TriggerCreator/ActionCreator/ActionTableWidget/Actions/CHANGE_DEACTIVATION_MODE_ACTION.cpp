...
void ActionTableWidget::CHANGE_DEACTIVATION_MODE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_DEACTIVATION_MODE")
	{
		if(_index.column() == VALUE)
		{
			switch(_index.row())
			{
				...
				case TARGET_PROPERTY:
				{
					...
					{
						_editor = UniversalDelegate::ACTIVATABLE_OBJECT_EDITOR;
						return;
					}
...
bool ActionTableWidget::CHANGE_DEACTIVATION_MODE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_DEACTIVATION_MODE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Deactivation mode"); 
		...
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Activatable object"); 
		}
		return true;
	}    
	return false;
...
QString ActionTableWidget::CHANGE_DEACTIVATION_MODE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DEACTIVATION_MODE")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_DEACTIVATION_MODE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DEACTIVATION_MODE")
	{ 
		UpdateActivatableObject(TARGET_PROPERTY);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::CHANGE_DEACTIVATION_MODE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DEACTIVATION_MODE")
	{
...
void ActionTableWidget::CHANGE_DEACTIVATION_MODE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DEACTIVATION_MODE")
	{
		if(item(_index.row(), ARG_MODE)->text() == "VARIABLE")
		{
			...
			{
				ActivatableObjectDropEvent(_it, _index, _suffix, _fileName);
			}
		}
...
void ActionCreator::CHANGE_DEACTIVATION_MODE_SetAction(int _actionType)
{
	if(_actionType == ActivatableObject::CHANGE_DEACTIVATION_MODE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_DEACTIVATION_MODE");

		ChangeDeactivationModeAction* derived = dynamic_cast<ChangeDeactivationModeAction*>(action);
...
void ActionCreator::CHANGE_DEACTIVATION_MODE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_DEACTIVATION_MODE")
	{
		action = Trigger::_CreateAction(ActivatableObject::CHANGE_DEACTIVATION_MODE_MESSAGE);
