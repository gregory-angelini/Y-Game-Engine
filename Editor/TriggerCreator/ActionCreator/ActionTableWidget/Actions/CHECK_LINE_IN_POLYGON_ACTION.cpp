...
void ActionTableWidget::CHECK_LINE_IN_POLYGON_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHECK_LINE_IN_POLYGON")
	{
...
bool ActionTableWidget::CHECK_LINE_IN_POLYGON_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHECK_LINE_IN_POLYGON") 
	{
		...
		{ 
			...
			SetCellText(ARG4_PROPERTY, CONTENT, "Intersection point"); 
		...
		{ 
			...
			if(item(ARG5_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG5_PROPERTY, CONTENT, "Target polygon"); 
				SetCellText(ARG5_PROPERTY, DATA_TYPE, "Integer"); 
			}
			else
			{
				SetCellText(ARG5_PROPERTY, CONTENT, "Target polygon"); 
				SetCellText(ARG5_PROPERTY, DATA_TYPE, "Integer variable"); 
			}
		}
...
QString ActionTableWidget::CHECK_LINE_IN_POLYGON_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_POLYGON")
	{ 
		QString name;
...
bool ActionTableWidget::CHECK_LINE_IN_POLYGON_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_POLYGON")
	{ 
...
bool ActionTableWidget::CHECK_LINE_IN_POLYGON_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_POLYGON")
	{
...
bool ActionTableWidget::CHECK_LINE_IN_POLYGON_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_POLYGON")
	{
...
bool ActionTableWidget::CHECK_LINE_IN_POLYGON_UpdateArg3VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_POLYGON")
	{
...
bool ActionTableWidget::CHECK_LINE_IN_POLYGON_UpdateArg4VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_POLYGON")
	{
...
bool ActionTableWidget::CHECK_LINE_IN_POLYGON_UpdateArg5VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_POLYGON")
	{
		UpdateVariable(ARG5_PROPERTY, Variable<int32>::INTEGER_TYPE); 
		return true;
	}
	return false;
}

void ActionTableWidget::CHECK_LINE_IN_POLYGON_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_LINE_IN_POLYGON")
	{
...
void ActionCreator::CHECK_LINE_IN_POLYGON_SetAction(int _actionType)
{
	if(_actionType == PolygonalSurface::CHECK_LINE_IN_POLYGON_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHECK_LINE_IN_POLYGON");

		CheckLineInPolygonAction* derived = dynamic_cast<CheckLineInPolygonAction*>(action);
		...
		if(derived)
		{
			...
			if(derived->GetArgState(AbstractAction::KEY, AbstractAction::ARG5))
			{
				...
				actionTable->SetCellText(ActionTableWidget::ARG5_PROPERTY, ActionTableWidget::VALUE, QString::number(derived->GetIntegerKeyArg(AbstractAction::ARG5)));
			}
...
void ActionCreator::CHECK_LINE_IN_POLYGON_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHECK_LINE_IN_POLYGON")
	{
		action = Trigger::_CreateAction(PolygonalSurface::CHECK_LINE_IN_POLYGON_MESSAGE);
		...
		if(action)
		{
			...
			{
				action->SetIntegerKeyArg(AbstractAction::ARG5, actionTable->GetCellText(ActionTableWidget::ARG5_PROPERTY, ActionTableWidget::VALUE).toInt());
			}
