...
void ActionTableWidget::CHANGE_PROJECTION_MODE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_PROJECTION_MODE")
	{
...
bool ActionTableWidget::CHANGE_PROJECTION_MODE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_PROJECTION_MODE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Projection mode"); 
...
QString ActionTableWidget::CHANGE_PROJECTION_MODE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_PROJECTION_MODE")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_PROJECTION_MODE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_PROJECTION_MODE")
	{ 
...
bool ActionTableWidget::CHANGE_PROJECTION_MODE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_PROJECTION_MODE")
	{
...
void ActionTableWidget::CHANGE_PROJECTION_MODE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_PROJECTION_MODE")
	{
...
void ActionCreator::CHANGE_PROJECTION_MODE_SetAction(int _actionType)
{
	if(_actionType == Camera3D::CHANGE_PROJECTION_MODE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_PROJECTION_MODE");

		ChangeProjectionModeAction* derived = dynamic_cast<ChangeProjectionModeAction*>(action);
...
void ActionCreator::CHANGE_PROJECTION_MODE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_PROJECTION_MODE")
	{
		action = Trigger::_CreateAction(Camera3D::CHANGE_PROJECTION_MODE_MESSAGE);
