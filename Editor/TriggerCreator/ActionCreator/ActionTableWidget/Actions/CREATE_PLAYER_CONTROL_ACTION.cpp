...
void ActionTableWidget::CREATE_PLAYER_CONTROL_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_PLAYER_CONTROL")
	{
...
bool ActionTableWidget::CREATE_PLAYER_CONTROL_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_PLAYER_CONTROL") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Player control name"); 
...
QString ActionTableWidget::CREATE_PLAYER_CONTROL_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_PLAYER_CONTROL")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_PLAYER_CONTROL_UpdateArg1KeyPropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_PLAYER_CONTROL")
	{ 
...
bool ActionTableWidget::CREATE_PLAYER_CONTROL_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_PLAYER_CONTROL")
	{
...
void ActionTableWidget::CREATE_PLAYER_CONTROL_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_PLAYER_CONTROL")
	{
...
void ActionCreator::CREATE_PLAYER_CONTROL_SetAction(int _actionType)
{
	if(_actionType == PlayerControl::CREATE_PLAYER_CONTROL_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_PLAYER_CONTROL");

		CreatePlayerControlAction* derived = dynamic_cast<CreatePlayerControlAction*>(action);
...
void ActionCreator::CREATE_PLAYER_CONTROL_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_PLAYER_CONTROL")
	{
		action = Trigger::_CreateAction(PlayerControl::CREATE_PLAYER_CONTROL_MESSAGE);
