...
void ActionTableWidget::SAVE_TO_FILE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "SAVE_TO_FILE")
	{
...
bool ActionTableWidget::SAVE_TO_FILE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "SAVE_TO_FILE") 
	{
...
QString ActionTableWidget::SAVE_TO_FILE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SAVE_TO_FILE")
	{ 
		QString name;
...
bool ActionTableWidget::SAVE_TO_FILE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SAVE_TO_FILE")
	{ 
...
void ActionTableWidget::SAVE_TO_FILE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SAVE_TO_FILE")
	{
...
void ActionCreator::SAVE_TO_FILE_SetAction(int _actionType)
{
	if(_actionType == AbstractObject::SAVE_TO_FILE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "SAVE_TO_FILE");

		SaveToFileAction* derived = dynamic_cast<SaveToFileAction*>(action);
...
void ActionCreator::SAVE_TO_FILE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "SAVE_TO_FILE")
	{
		action = Trigger::_CreateAction(AbstractObject::SAVE_TO_FILE_MESSAGE);
