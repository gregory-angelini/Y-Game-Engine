...
void ActionTableWidget::GET_ROTATION_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_ROTATION")
	{
...
bool ActionTableWidget::GET_ROTATION_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_ROTATION") 
	{
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "Absolute/Local"); 
...
QString ActionTableWidget::GET_ROTATION_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_ROTATION")
	{ 
		QString name;
...
bool ActionTableWidget::GET_ROTATION_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_ROTATION")
	{ 
...
bool ActionTableWidget::GET_ROTATION_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_ROTATION")
	{
...
bool ActionTableWidget::GET_ROTATION_UpdateArg2VariablePropertyStatus(void)
{
...
void ActionTableWidget::GET_ROTATION_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_ROTATION")
	{
...
void ActionCreator::GET_ROTATION_SetAction(int _actionType)
{
	if(_actionType == RotatableObject::GET_ROTATION_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_ROTATION");

		GetRotationAction* derived = dynamic_cast<GetRotationAction*>(action);
...
void ActionCreator::GET_ROTATION_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_ROTATION")
	{
		action = Trigger::_CreateAction(RotatableObject::GET_ROTATION_MESSAGE);
