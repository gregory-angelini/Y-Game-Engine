...
void ActionTableWidget::ENABLE_COLLISION_CHECKING_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ENABLE_COLLISION_CHECKING")
	{
...
bool ActionTableWidget::ENABLE_COLLISION_CHECKING_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ENABLE_COLLISION_CHECKING") 
	{
...
QString ActionTableWidget::ENABLE_COLLISION_CHECKING_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_COLLISION_CHECKING")
	{ 
		QString name;
...
bool ActionTableWidget::ENABLE_COLLISION_CHECKING_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_COLLISION_CHECKING")
	{ 
...
bool ActionTableWidget::ENABLE_COLLISION_CHECKING_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_COLLISION_CHECKING")
	{
...
void ActionTableWidget::ENABLE_COLLISION_CHECKING_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_COLLISION_CHECKING")
	{
...
void ActionCreator::ENABLE_COLLISION_CHECKING_SetAction(int _actionType)
{
	if(_actionType == PolygonalSurface::ENABLE_COLLISION_CHECKING_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ENABLE_COLLISION_CHECKING");

		EnableCollisionCheckingAction* derived = dynamic_cast<EnableCollisionCheckingAction*>(action);
...
void ActionCreator::ENABLE_COLLISION_CHECKING_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ENABLE_COLLISION_CHECKING")
	{
		action = Trigger::_CreateAction(PolygonalSurface::ENABLE_COLLISION_CHECKING_MESSAGE);
