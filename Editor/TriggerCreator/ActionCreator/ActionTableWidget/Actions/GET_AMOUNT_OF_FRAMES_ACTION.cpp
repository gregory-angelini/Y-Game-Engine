...
bool ActionTableWidget::GET_AMOUNT_OF_FRAMES_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_AMOUNT_OF_FRAMES") 
	{
...
QString ActionTableWidget::GET_AMOUNT_OF_FRAMES_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_AMOUNT_OF_FRAMES")
	{ 
		QString name;
...
bool ActionTableWidget::GET_AMOUNT_OF_FRAMES_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_AMOUNT_OF_FRAMES")
	{ 
...
bool ActionTableWidget::GET_AMOUNT_OF_FRAMES_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_AMOUNT_OF_FRAMES")
	{
...
void ActionTableWidget::GET_AMOUNT_OF_FRAMES_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_AMOUNT_OF_FRAMES")
	{
...
void ActionCreator::GET_AMOUNT_OF_FRAMES_SetAction(int _actionType)
{
	if(_actionType == AbstractAnimation::GET_AMOUNT_OF_FRAMES_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_AMOUNT_OF_FRAMES");

		GetAmountOfFramesAction* derived = dynamic_cast<GetAmountOfFramesAction*>(action);
...
void ActionCreator::GET_AMOUNT_OF_FRAMES_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_AMOUNT_OF_FRAMES")
	{
		action = Trigger::_CreateAction(AbstractAnimation::GET_AMOUNT_OF_FRAMES_MESSAGE);
