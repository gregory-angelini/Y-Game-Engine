...
void ActionTableWidget::CHANGE_DELTA_SCALE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_DELTA_SCALE")
	{
...
bool ActionTableWidget::CHANGE_DELTA_SCALE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_DELTA_SCALE") 
	{
		...
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Logic object"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::CHANGE_DELTA_SCALE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DELTA_SCALE")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_DELTA_SCALE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DELTA_SCALE")
	{ 
		UpdateLogicObject(TARGET_PROPERTY);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::CHANGE_DELTA_SCALE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DELTA_SCALE")
	{
...
void ActionTableWidget::CHANGE_DELTA_SCALE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DELTA_SCALE")
	{
...
void ActionCreator::CHANGE_DELTA_SCALE_SetAction(int _actionType)
{
	if(_actionType == LogicObject::CHANGE_DELTA_SCALE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_DELTA_SCALE");

		ChangeDeltaScaleAction* derived = dynamic_cast<ChangeDeltaScaleAction*>(action);
...
void ActionCreator::CHANGE_DELTA_SCALE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_DELTA_SCALE")
	{
		action = Trigger::_CreateAction(LogicObject::CHANGE_DELTA_SCALE_MESSAGE);
