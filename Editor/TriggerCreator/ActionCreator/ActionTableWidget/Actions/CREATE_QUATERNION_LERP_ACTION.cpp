...
void ActionTableWidget::CREATE_QUATERNION_LERP_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_QUATERNION_LERP")
	{
...
bool ActionTableWidget::CREATE_QUATERNION_LERP_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_QUATERNION_LERP") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Quaternion lerp name"); 
...
QString ActionTableWidget::CREATE_QUATERNION_LERP_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_QUATERNION_LERP")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_QUATERNION_LERP_UpdateArg1KeyPropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_QUATERNION_LERP")
	{ 
...
bool ActionTableWidget::CREATE_QUATERNION_LERP_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_QUATERNION_LERP")
	{
...
void ActionTableWidget::CREATE_QUATERNION_LERP_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_QUATERNION_LERP")
	{
...
void ActionCreator::CREATE_QUATERNION_LERP_SetAction(int _actionType)
{
	if(_actionType == QuaternionLerp::CREATE_QUATERNION_LERP_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_QUATERNION_LERP");

		CreateQuaternionLerpAction* derived = dynamic_cast<CreateQuaternionLerpAction*>(action);
...
void ActionCreator::CREATE_QUATERNION_LERP_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_QUATERNION_LERP")
	{
		action = Trigger::_CreateAction(QuaternionLerp::CREATE_QUATERNION_LERP_MESSAGE);
