...
void ActionTableWidget::ENABLE_BACK_FACE_CULLING_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ENABLE_BACK_FACE_CULLING")
	{
...
bool ActionTableWidget::ENABLE_BACK_FACE_CULLING_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ENABLE_BACK_FACE_CULLING") 
	{
		...
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Graphics surface"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::ENABLE_BACK_FACE_CULLING_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_BACK_FACE_CULLING")
	{ 
		QString name;
...
bool ActionTableWidget::ENABLE_BACK_FACE_CULLING_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_BACK_FACE_CULLING")
	{ 
...
bool ActionTableWidget::ENABLE_BACK_FACE_CULLING_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_BACK_FACE_CULLING")
	{
...
void ActionTableWidget::ENABLE_BACK_FACE_CULLING_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_BACK_FACE_CULLING")
	{
...
void ActionCreator::ENABLE_BACK_FACE_CULLING_SetAction(int _actionType)
{
	if(_actionType == GraphicsSurface::ENABLE_BACK_FACE_CULLING_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ENABLE_BACK_FACE_CULLING");

		EnableBackFaceCullingAction* derived = dynamic_cast<EnableBackFaceCullingAction*>(action);
...
void ActionCreator::ENABLE_BACK_FACE_CULLING_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ENABLE_BACK_FACE_CULLING")
	{
		action = Trigger::_CreateAction(GraphicsSurface::ENABLE_BACK_FACE_CULLING_MESSAGE);
