...
void ActionTableWidget::GET_CONTROL_VECTORS_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_CONTROL_VECTORS")
	{
...
bool ActionTableWidget::GET_CONTROL_VECTORS_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_CONTROL_VECTORS") 
	{
...
QString ActionTableWidget::GET_CONTROL_VECTORS_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CONTROL_VECTORS")
	{ 
		QString name;
...
bool ActionTableWidget::GET_CONTROL_VECTORS_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CONTROL_VECTORS")
	{ 
...
bool ActionTableWidget::GET_CONTROL_VECTORS_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CONTROL_VECTORS")
	{
...
bool ActionTableWidget::GET_CONTROL_VECTORS_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CONTROL_VECTORS")
	{
...
void ActionTableWidget::GET_CONTROL_VECTORS_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CONTROL_VECTORS")
	{
...
void ActionCreator::GET_CONTROL_VECTORS_SetAction(int _actionType)
{
	if(_actionType == VectorLerp::GET_CONTROL_VECTORS_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_CONTROL_VECTORS");

		GetControlVectorsAction* derived = dynamic_cast<GetControlVectorsAction*>(action);
...
void ActionCreator::GET_CONTROL_VECTORS_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_CONTROL_VECTORS")
	{
		action = Trigger::_CreateAction(VectorLerp::GET_CONTROL_VECTORS_MESSAGE);
