...
void ActionTableWidget::CHANGE_ANIMATION_FRAME_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_ANIMATION_FRAME")
	{
...
bool ActionTableWidget::CHANGE_ANIMATION_FRAME_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_ANIMATION_FRAME") 
	{
		...
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Abstract animation"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::CHANGE_ANIMATION_FRAME_CreateAutoName(void)
{
...
bool ActionTableWidget::CHANGE_ANIMATION_FRAME_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ANIMATION_FRAME")
	{ 
...
bool ActionTableWidget::CHANGE_ANIMATION_FRAME_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ANIMATION_FRAME")
	{
...
void ActionTableWidget::CHANGE_ANIMATION_FRAME_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ANIMATION_FRAME")
	{
...
void ActionCreator::CHANGE_ANIMATION_FRAME_SetAction(int _actionType)
{
	if(_actionType == AbstractAnimation::CHANGE_ANIMATION_FRAME_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_ANIMATION_FRAME");

		ChangeAnimationFrameAction* derived = dynamic_cast<ChangeAnimationFrameAction*>(action);
...
void ActionCreator::CHANGE_ANIMATION_FRAME_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_ANIMATION_FRAME")
	{
		action = Trigger::_CreateAction(AbstractAnimation::CHANGE_ANIMATION_FRAME_MESSAGE);
