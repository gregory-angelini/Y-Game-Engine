...
void ActionTableWidget::CHECK_OBJECT_EXIST_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHECK_OBJECT_EXIST")
	{
...
bool ActionTableWidget::CHECK_OBJECT_EXIST_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHECK_OBJECT_EXIST") 
	{
...
QString ActionTableWidget::CHECK_OBJECT_EXIST_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_OBJECT_EXIST")
	{ 
		QString name;
...
bool ActionTableWidget::CHECK_OBJECT_EXIST_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_OBJECT_EXIST")
	{ 
...
bool ActionTableWidget::CHECK_OBJECT_EXIST_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_OBJECT_EXIST")
	{
...
void ActionTableWidget::CHECK_OBJECT_EXIST_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_OBJECT_EXIST")
	{
...
void ActionCreator::CHECK_OBJECT_EXIST_SetAction(int _actionType)
{
	if(_actionType == AssetLibrary::CHECK_OBJECT_EXIST_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHECK_OBJECT_EXIST");

		CheckObjectExistAction* derived = dynamic_cast<CheckObjectExistAction*>(action);
...
void ActionCreator::CHECK_OBJECT_EXIST_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHECK_OBJECT_EXIST")
	{
		action = Trigger::_CreateAction(AssetLibrary::CHECK_OBJECT_EXIST_MESSAGE);
