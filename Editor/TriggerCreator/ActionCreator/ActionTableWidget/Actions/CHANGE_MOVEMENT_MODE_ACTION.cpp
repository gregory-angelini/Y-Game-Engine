...
void ActionTableWidget::CHANGE_MOVEMENT_MODE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_MOVEMENT_MODE")
	{
...
bool ActionTableWidget::CHANGE_MOVEMENT_MODE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_MOVEMENT_MODE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Movement mode"); 
...
QString ActionTableWidget::CHANGE_MOVEMENT_MODE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_MOVEMENT_MODE")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_MOVEMENT_MODE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_MOVEMENT_MODE")
	{ 
...
bool ActionTableWidget::CHANGE_MOVEMENT_MODE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_MOVEMENT_MODE")
	{
...
void ActionTableWidget::CHANGE_MOVEMENT_MODE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_MOVEMENT_MODE")
	{
...
void ActionCreator::CHANGE_MOVEMENT_MODE_SetAction(int _actionType)
{
	if(_actionType == Camera3D::CHANGE_MOVEMENT_MODE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_MOVEMENT_MODE");

		ChangeMovementModeAction* derived = dynamic_cast<ChangeMovementModeAction*>(action);
...
void ActionCreator::CHANGE_MOVEMENT_MODE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_MOVEMENT_MODE")
	{
		action = Trigger::_CreateAction(Camera3D::CHANGE_MOVEMENT_MODE_MESSAGE);
