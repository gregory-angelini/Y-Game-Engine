...
void ActionTableWidget::GET_SCALE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_SCALE")
	{
...
bool ActionTableWidget::GET_SCALE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_SCALE") 
	{
		...
		{ 
			...
			SetCellText(ARG1_PROPERTY, DATA_TYPE, "Scalable object"); 
		}
		...
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Scalable object"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::GET_SCALE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_SCALE")
	{ 
		QString name;
...
bool ActionTableWidget::GET_SCALE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_SCALE")
	{ 
		UpdateScalableObject(TARGET_PROPERTY);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::GET_SCALE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_SCALE")
	{
		UpdateScalableObject(ARG1_PROPERTY); 
		return true;
	}
	return false;
}
...
bool ActionTableWidget::GET_SCALE_UpdateArg2VariablePropertyStatus(void)
{
...
void ActionTableWidget::GET_SCALE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_SCALE")
	{
...
void ActionCreator::GET_SCALE_SetAction(int _actionType)
{
	if(_actionType == ScalableObject::GET_SCALE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_SCALE");

		GetScaleAction* derived = dynamic_cast<GetScaleAction*>(action);
...
void ActionCreator::GET_SCALE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_SCALE")
	{
		action = Trigger::_CreateAction(ScalableObject::GET_SCALE_MESSAGE);
