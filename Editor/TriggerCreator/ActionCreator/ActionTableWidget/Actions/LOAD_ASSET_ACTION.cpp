...
void ActionTableWidget::LOAD_ASSET_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "LOAD_ASSET")
	{
...
bool ActionTableWidget::LOAD_ASSET_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "LOAD_ASSET") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Asset name"); 
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "Asset type"); 
...
QString ActionTableWidget::LOAD_ASSET_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "LOAD_ASSET")
	{ 
		QString name;
...
bool ActionTableWidget::LOAD_ASSET_UpdateArg1KeyPropertyStatus(void)
{ 
	if(item(TYPE_PROPERTY, VALUE)->text() == "LOAD_ASSET")
	{ 
...
bool ActionTableWidget::LOAD_ASSET_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "LOAD_ASSET")
	{
...
bool ActionTableWidget::LOAD_ASSET_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "LOAD_ASSET")
	{
...
void ActionTableWidget::LOAD_ASSET_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "LOAD_ASSET")
	{
...
void ActionCreator::LOAD_ASSET_SetAction(int _actionType)
{
	if(_actionType == AssetLibrary::LOAD_ASSET_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "LOAD_ASSET");

		LoadAssetAction* derived = dynamic_cast<LoadAssetAction*>(action);
...
void ActionCreator::LOAD_ASSET_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "LOAD_ASSET")
	{
		action = Trigger::_CreateAction(AssetLibrary::LOAD_ASSET_MESSAGE);
