...
void ActionTableWidget::MOVE_AROUND_PIVOT_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "MOVE_AROUND_PIVOT")
	{
...
bool ActionTableWidget::MOVE_AROUND_PIVOT_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "MOVE_AROUND_PIVOT") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Rotation"); 
				SetCellText(ARG1_PROPERTY, DATA_TYPE, "Quaternion");
			}
			else
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Rotation"); 
				SetCellText(ARG1_PROPERTY, DATA_TYPE, "Rotatable object"); 
			}
		}
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "Pivot"); 
		...
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Translatable object"); 
		} 
		return true;
	}  
	return false;
...
QString ActionTableWidget::MOVE_AROUND_PIVOT_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "MOVE_AROUND_PIVOT")
	{ 
		QString name;
...
bool ActionTableWidget::MOVE_AROUND_PIVOT_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "MOVE_AROUND_PIVOT")
	{ 
		UpdateTranslatableObject(TARGET_PROPERTY);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::MOVE_AROUND_PIVOT_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "MOVE_AROUND_PIVOT")
	{
		UpdateRotatableObject(ARG1_PROPERTY);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::MOVE_AROUND_PIVOT_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "MOVE_AROUND_PIVOT")
	{
...
void ActionTableWidget::MOVE_AROUND_PIVOT_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "MOVE_AROUND_PIVOT")
	{
...
void ActionCreator::MOVE_AROUND_PIVOT_SetAction(int _actionType)
{
	if(_actionType == TranslatableObject::MOVE_AROUND_PIVOT_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "MOVE_AROUND_PIVOT");

		MoveAroundPivotAction* derived = dynamic_cast<MoveAroundPivotAction*>(action);
		...
		if(derived)
		{
			if(derived->GetArgState(AbstractAction::KEY, AbstractAction::ARG1))
			{
				...
				actionTable->SetCellText(ActionTableWidget::ARG1_PROPERTY, ActionTableWidget::VALUE, QuaternionToString(derived->GetQuaternionKeyArg(AbstractAction::ARG1)));
			}
...
void ActionCreator::MOVE_AROUND_PIVOT_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "MOVE_AROUND_PIVOT")
	{
		action = Trigger::_CreateAction(TranslatableObject::MOVE_AROUND_PIVOT_MESSAGE);
		...
		if(action)
		{
			...
			{
				action->SetQuaternionKeyArg(AbstractAction::ARG1, StringToQuaternion(actionTable->GetCellText(ActionTableWidget::ARG1_PROPERTY, ActionTableWidget::VALUE)));
			}
