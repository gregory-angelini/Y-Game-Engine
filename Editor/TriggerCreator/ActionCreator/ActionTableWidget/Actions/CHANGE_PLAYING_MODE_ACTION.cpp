...
void ActionTableWidget::CHANGE_PLAYING_MODE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_PLAYING_MODE")
	{
...
bool ActionTableWidget::CHANGE_PLAYING_MODE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_PLAYING_MODE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Playing mode"); 
...
QString ActionTableWidget::CHANGE_PLAYING_MODE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_PLAYING_MODE")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_PLAYING_MODE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_PLAYING_MODE")
	{ 
...
bool ActionTableWidget::CHANGE_PLAYING_MODE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_PLAYING_MODE")
	{
...
void ActionTableWidget::CHANGE_PLAYING_MODE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_PLAYING_MODE")
	{
...
void ActionCreator::CHANGE_PLAYING_MODE_SetAction(int _actionType)
{
	if(_actionType == PlayingObject::CHANGE_PLAYING_MODE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_PLAYING_MODE");

		ChangePlayingModeAction* derived = dynamic_cast<ChangePlayingModeAction*>(action);
...
void ActionCreator::CHANGE_PLAYING_MODE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_PLAYING_MODE")
	{
		action = Trigger::_CreateAction(PlayingObject::CHANGE_PLAYING_MODE_MESSAGE);
