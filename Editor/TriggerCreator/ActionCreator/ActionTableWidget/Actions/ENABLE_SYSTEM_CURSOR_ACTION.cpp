...
void ActionTableWidget::ENABLE_SHOW_CURSOR_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ENABLE_SHOW_CURSOR")
	{
...
bool ActionTableWidget::ENABLE_SHOW_CURSOR_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ENABLE_SHOW_CURSOR") 
	{
...
QString ActionTableWidget::ENABLE_SHOW_CURSOR_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_SHOW_CURSOR")
	{ 
		QString name;
...
bool ActionTableWidget::ENABLE_SHOW_CURSOR_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_SHOW_CURSOR")
	{
...
void ActionTableWidget::ENABLE_SHOW_CURSOR_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_SHOW_CURSOR")
	{
...
void ActionCreator::ENABLE_SHOW_CURSOR_SetAction(int _actionType)
{
	if(_actionType == Mouse::ENABLE_SHOW_CURSOR_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ENABLE_SHOW_CURSOR");

		EnableShowCursorAction* derived = dynamic_cast<EnableShowCursorAction*>(action);
...
void ActionCreator::ENABLE_SHOW_CURSOR_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ENABLE_SHOW_CURSOR")
	{
		action = Trigger::_CreateAction(Mouse::ENABLE_SHOW_CURSOR_MESSAGE);
