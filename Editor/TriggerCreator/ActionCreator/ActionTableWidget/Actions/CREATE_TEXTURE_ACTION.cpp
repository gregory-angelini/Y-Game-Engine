...
void ActionTableWidget::CREATE_TEXTURE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_TEXTURE")
	{
...
bool ActionTableWidget::CREATE_TEXTURE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_TEXTURE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Texture name"); 
...
QString ActionTableWidget::CREATE_TEXTURE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TEXTURE")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_TEXTURE_UpdateArg1KeyPropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TEXTURE")
	{ 
...
bool ActionTableWidget::CREATE_TEXTURE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TEXTURE")
	{
...
void ActionTableWidget::CREATE_TEXTURE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TEXTURE")
	{
...
void ActionCreator::CREATE_TEXTURE_SetAction(int _actionType)
{
	if(_actionType == Texture2D::CREATE_TEXTURE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_TEXTURE");

		CreateTextureAction* derived = dynamic_cast<CreateTextureAction*>(action);
...
void ActionCreator::CREATE_TEXTURE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_TEXTURE")
	{
		action = Trigger::_CreateAction(Texture2D::CREATE_TEXTURE_MESSAGE);
