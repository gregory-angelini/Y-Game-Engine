...
void ActionTableWidget::REPLACE_SCALE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "REPLACE_SCALE")
	{
...
bool ActionTableWidget::REPLACE_SCALE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "REPLACE_SCALE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Scale"); 
...
QString ActionTableWidget::REPLACE_SCALE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REPLACE_SCALE")
	{ 
		QString name;
...
bool ActionTableWidget::REPLACE_SCALE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REPLACE_SCALE")
	{ 
...
bool ActionTableWidget::REPLACE_SCALE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REPLACE_SCALE")
	{
...
void ActionTableWidget::REPLACE_SCALE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REPLACE_SCALE")
	{
...
void ActionCreator::REPLACE_SCALE_SetAction(int _actionType)
{
	if(_actionType == ScalableObject::REPLACE_SCALE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "REPLACE_SCALE");

		ReplaceScaleAction* derived = dynamic_cast<ReplaceScaleAction*>(action);
...
void ActionCreator::REPLACE_SCALE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "REPLACE_SCALE")
	{
		action = Trigger::_CreateAction(ScalableObject::REPLACE_SCALE_MESSAGE);
