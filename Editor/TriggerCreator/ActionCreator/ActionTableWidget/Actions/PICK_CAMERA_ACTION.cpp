...
void ActionTableWidget::PICK_CAMERA_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "PICK_CAMERA")
	{
...
bool ActionTableWidget::PICK_CAMERA_UpdateRow(const QModelIndex& _index, const QString& _type)
{
...
QString ActionTableWidget::PICK_CAMERA_CreateAutoName(void)
{
...
bool ActionTableWidget::PICK_CAMERA_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "PICK_CAMERA")
	{ 
...
void ActionTableWidget::PICK_CAMERA_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "PICK_CAMERA")
	{
...
void ActionCreator::PICK_CAMERA_SetAction(int _actionType)
{
	if(_actionType == Camera3D::PICK_CAMERA_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "PICK_CAMERA");

		PickCameraAction* derived = dynamic_cast<PickCameraAction*>(action);
...
void ActionCreator::PICK_CAMERA_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "PICK_CAMERA")
	{
		action = Trigger::_CreateAction(Camera3D::PICK_CAMERA_MESSAGE);
