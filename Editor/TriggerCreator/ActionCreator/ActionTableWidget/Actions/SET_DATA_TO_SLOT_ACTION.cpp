...
void ActionTableWidget::SET_DATA_TO_SLOT_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "SET_DATA_TO_SLOT")
	{
...
bool ActionTableWidget::SET_DATA_TO_SLOT_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "SET_DATA_TO_SLOT") 
	{
		...
		else if(_index.row() == ARG1_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(ARG1_PROPERTY, CONTENT, "Source"); 
...
QString ActionTableWidget::SET_DATA_TO_SLOT_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SET_DATA_TO_SLOT")
	{ 
		QString name;
...
bool ActionTableWidget::SET_DATA_TO_SLOT_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SET_DATA_TO_SLOT")
	{ 
...
bool ActionTableWidget::SET_DATA_TO_SLOT_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SET_DATA_TO_SLOT")
	{
...
bool ActionTableWidget::SET_DATA_TO_SLOT_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SET_DATA_TO_SLOT")
	{
...
void ActionTableWidget::SET_DATA_TO_SLOT_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SET_DATA_TO_SLOT")
	{
...
void ActionCreator::SET_DATA_TO_SLOT_SetAction(int _actionType)
{
	if(_actionType == Set::SET_DATA_TO_SLOT_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "SET_DATA_TO_SLOT");

		SetDataToSlotAction* derived = dynamic_cast<SetDataToSlotAction*>(action);
...
void ActionCreator::SET_DATA_TO_SLOT_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "SET_DATA_TO_SLOT")
	{
		action = Trigger::_CreateAction(Set::SET_DATA_TO_SLOT_MESSAGE);
