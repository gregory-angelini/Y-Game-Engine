...
void ActionTableWidget::CHANGE_INPUT_TIMING_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_INPUT_TIMING")
	{
...
bool ActionTableWidget::CHANGE_INPUT_TIMING_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_INPUT_TIMING") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Duration (in sec)"); 
...
QString ActionTableWidget::CHANGE_INPUT_TIMING_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_INPUT_TIMING")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_INPUT_TIMING_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_INPUT_TIMING")
	{ 
...
bool ActionTableWidget::CHANGE_INPUT_TIMING_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_INPUT_TIMING")
	{
...
void ActionTableWidget::CHANGE_INPUT_TIMING_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_INPUT_TIMING")
	{
...
void ActionCreator::CHANGE_INPUT_TIMING_SetAction(int _actionType)
{
	if(_actionType == PlayerControl::CHANGE_INPUT_TIMING_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_INPUT_TIMING");

		ChangeInputTimingAction* derived = dynamic_cast<ChangeInputTimingAction*>(action);
...
void ActionCreator::CHANGE_INPUT_TIMING_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_INPUT_TIMING")
	{
		action = Trigger::_CreateAction(PlayerControl::CHANGE_INPUT_TIMING_MESSAGE);
