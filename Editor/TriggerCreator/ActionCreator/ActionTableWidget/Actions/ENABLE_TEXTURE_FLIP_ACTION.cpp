...
void ActionTableWidget::ENABLE_TEXTURE_FLIP_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ENABLE_TEXTURE_FLIP")
	{
...
bool ActionTableWidget::ENABLE_TEXTURE_FLIP_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ENABLE_TEXTURE_FLIP") 
	{
...
QString ActionTableWidget::ENABLE_TEXTURE_FLIP_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_TEXTURE_FLIP")
	{ 
		QString name;
...
bool ActionTableWidget::ENABLE_TEXTURE_FLIP_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_TEXTURE_FLIP")
	{ 
...
bool ActionTableWidget::ENABLE_TEXTURE_FLIP_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_TEXTURE_FLIP")
	{
...
bool ActionTableWidget::ENABLE_TEXTURE_FLIP_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_TEXTURE_FLIP")
	{
...
void ActionTableWidget::ENABLE_TEXTURE_FLIP_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_TEXTURE_FLIP")
	{
...
void ActionCreator::ENABLE_TEXTURE_FLIP_SetAction(int _actionType)
{
	if(_actionType == TexturingSurface::ENABLE_TEXTURE_FLIP_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ENABLE_TEXTURE_FLIP");

		EnableTextureFlipAction* derived = dynamic_cast<EnableTextureFlipAction*>(action);
...
void ActionCreator::ENABLE_TEXTURE_FLIP_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ENABLE_TEXTURE_FLIP")
	{
		action = Trigger::_CreateAction(TexturingSurface::ENABLE_TEXTURE_FLIP_MESSAGE);
