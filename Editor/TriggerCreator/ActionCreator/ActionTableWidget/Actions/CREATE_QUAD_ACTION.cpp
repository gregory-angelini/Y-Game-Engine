...
void ActionTableWidget::CREATE_QUAD_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_QUAD")
	{
		if(_index.column() == VALUE)
		{
			switch(_index.row())
			{
				...
				case TARGET_PROPERTY:
				{
					...
					{
						_editor = UniversalDelegate::MESH_EDITOR;
						return;
					}
...
bool ActionTableWidget::CREATE_QUAD_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_QUAD") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				...
				SetCellText(ARG1_PROPERTY, DATA_TYPE, "Vector2");
			}
			else
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Size"); 
				SetCellText(ARG1_PROPERTY, DATA_TYPE, "Vector2 variable"); 
			}
		}
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Mesh"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::CREATE_QUAD_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_QUAD")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_QUAD_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_QUAD")
	{ 
		UpdateMesh(TARGET_PROPERTY);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::CREATE_QUAD_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_QUAD")
	{
		UpdateVariable(ARG1_PROPERTY, Variable<Vector2>::VECTOR_2_TYPE);
		return true;
	}
	return false;
}
...
void ActionTableWidget::CREATE_QUAD_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_QUAD")
	{
		if(item(_index.row(), ARG_MODE)->text() == "VARIABLE")
		{
			...
			{
				MeshDropEvent(_it, _index, _suffix, _fileName);
			}
		}
...
void ActionCreator::CREATE_QUAD_SetAction(int _actionType)
{
	if(_actionType == Mesh::CREATE_QUAD_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_QUAD");

		CreateQuadAction* derived = dynamic_cast<CreateQuadAction*>(action);
		...
		if(derived)
		{
			...
			if(derived->GetArgState(AbstractAction::KEY, AbstractAction::ARG1))
			{
				...
				actionTable->SetCellText(ActionTableWidget::ARG1_PROPERTY, ActionTableWidget::VALUE, Vector2ToString(derived->GetVector2KeyArg(AbstractAction::ARG1)));
			}
...
void ActionCreator::CREATE_QUAD_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_QUAD")
	{
		action = Trigger::_CreateAction(Mesh::CREATE_QUAD_MESSAGE);
		...
		if(action)
		{
			...
			{
				action->SetVector2KeyArg(AbstractAction::ARG1, StringToVector2(actionTable->GetCellText(ActionTableWidget::ARG1_PROPERTY, ActionTableWidget::VALUE)));
			}
