...
void ActionTableWidget::CHECK_PROPERTY_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHECK_PROPERTY")
	{
...
bool ActionTableWidget::CHECK_PROPERTY_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHECK_PROPERTY") 
	{
...
QString ActionTableWidget::CHECK_PROPERTY_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_PROPERTY")
	{ 
		QString name;
...
bool ActionTableWidget::CHECK_PROPERTY_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_PROPERTY")
	{ 
...
bool ActionTableWidget::CHECK_PROPERTY_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_PROPERTY")
	{
...
bool ActionTableWidget::CHECK_PROPERTY_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_PROPERTY")
	{
...
void ActionTableWidget::CHECK_PROPERTY_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_PROPERTY")
	{
...
void ActionCreator::CHECK_PROPERTY_SetAction(int _actionType)
{
	if(_actionType == AbstractObject::CHECK_PROPERTY_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHECK_PROPERTY");

		CheckPropertyAction* derived = dynamic_cast<CheckPropertyAction*>(action);
...
void ActionCreator::CHECK_PROPERTY_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHECK_PROPERTY")
	{
		action = Trigger::_CreateAction(AbstractObject::CHECK_PROPERTY_MESSAGE);
