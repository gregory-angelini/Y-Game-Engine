...
void ActionTableWidget::ADD_TO_LIST_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ADD_TO_LIST")
	{
...
bool ActionTableWidget::ADD_TO_LIST_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ADD_TO_LIST") 
	{
		...
		{ 
			...
			if(item(ARG3_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG3_PROPERTY, CONTENT, "Copy mode"); 
...
QString ActionTableWidget::ADD_TO_LIST_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_TO_LIST")
	{ 
		QString name;
...
bool ActionTableWidget::ADD_TO_LIST_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_TO_LIST")
	{ 
...
bool ActionTableWidget::ADD_TO_LIST_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_TO_LIST")
	{
...
bool ActionTableWidget::ADD_TO_LIST_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_TO_LIST")
	{
...
bool ActionTableWidget::ADD_TO_LIST_UpdateArg3VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_TO_LIST")
	{
...
void ActionTableWidget::ADD_TO_LIST_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_TO_LIST")
	{
...
void ActionCreator::ADD_TO_LIST_SetAction(int _actionType)
{
	if(_actionType == AbstractList::ADD_TO_LIST_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ADD_TO_LIST");

		AddToListAction* derived = dynamic_cast<AddToListAction*>(action);
...
void ActionCreator::ADD_TO_LIST_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ADD_TO_LIST")
	{
		action = Trigger::_CreateAction(AbstractList::ADD_TO_LIST_MESSAGE);
