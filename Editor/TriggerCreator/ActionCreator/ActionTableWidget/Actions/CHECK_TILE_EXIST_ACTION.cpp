...
void ActionTableWidget::CHECK_TILE_EXIST_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHECK_TILE_EXIST")
	{
...
bool ActionTableWidget::CHECK_TILE_EXIST_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHECK_TILE_EXIST") 
	{
...
QString ActionTableWidget::CHECK_TILE_EXIST_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_TILE_EXIST")
	{ 
		QString name;
...
bool ActionTableWidget::CHECK_TILE_EXIST_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_TILE_EXIST")
	{ 
...
bool ActionTableWidget::CHECK_TILE_EXIST_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_TILE_EXIST")
	{
...
bool ActionTableWidget::CHECK_TILE_EXIST_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_TILE_EXIST")
	{ 
...
void ActionTableWidget::CHECK_TILE_EXIST_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_TILE_EXIST")
	{
...
void ActionCreator::CHECK_TILE_EXIST_SetAction(int _actionType)
{
	if(_actionType == TiledSpace3D::CHECK_TILE_EXIST_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHECK_TILE_EXIST");

		CheckTileExistAction* derived = dynamic_cast<CheckTileExistAction*>(action);
...
void ActionCreator::CHECK_TILE_EXIST_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHECK_TILE_EXIST")
	{
		action = Trigger::_CreateAction(TiledSpace3D::CHECK_TILE_EXIST_MESSAGE);
