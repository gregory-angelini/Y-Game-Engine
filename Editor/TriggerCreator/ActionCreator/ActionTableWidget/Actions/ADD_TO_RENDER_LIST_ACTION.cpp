...
void ActionTableWidget::ADD_TO_RENDER_LIST_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ADD_TO_RENDER_LIST")
	{
		if(_index.column() == VALUE)
		{
			switch(_index.row())
			{
				case ARG1_PROPERTY:
				{
					...
					{
						_editor = UniversalDelegate::GRAPHICS_OBJECT_EDITOR;
						return;
					}
...
bool ActionTableWidget::ADD_TO_RENDER_LIST_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ADD_TO_RENDER_LIST") 
	{
		...
		{ 
			...
			SetCellText(ARG1_PROPERTY, DATA_TYPE, "Graphics object"); 
		}
		else if(_index.row() == ARG2_PROPERTY && _index.column() == ARG_MODE)
		{ 
			SetDefaultProperty(ARG2_PROPERTY);
			SetCellText(ARG2_PROPERTY, CONTENT, "Anchor name"); 
			SetCellText(ARG2_PROPERTY, DATA_TYPE, "Graphics object"); 
		}
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Graphics scene"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::ADD_TO_RENDER_LIST_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_TO_RENDER_LIST")
	{ 
		QString name;
...
bool ActionTableWidget::ADD_TO_RENDER_LIST_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_TO_RENDER_LIST")
	{ 
...
bool ActionTableWidget::ADD_TO_RENDER_LIST_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_TO_RENDER_LIST")
	{
		UpdateGraphicsObject(ARG1_PROPERTY); 
		return true;
	}
	return false;
}
...
bool ActionTableWidget::ADD_TO_RENDER_LIST_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_TO_RENDER_LIST")
	{
		UpdateGraphicsObject(ARG2_PROPERTY); 
		return true;
	}
	return false;
}

void ActionTableWidget::ADD_TO_RENDER_LIST_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_TO_RENDER_LIST")
	{
		if(item(_index.row(), ARG_MODE)->text() == "VARIABLE")
		{
			...
			{
				GraphicsObjectDropEvent(_it, _index, _suffix, _fileName);
			}
...
void ActionCreator::ADD_TO_RENDER_LIST_SetAction(int _actionType)
{
	if(_actionType == GraphicsScene::ADD_TO_RENDER_LIST_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ADD_TO_RENDER_LIST");

		AddToRenderListAction* derived = dynamic_cast<AddToRenderListAction*>(action);
...
void ActionCreator::ADD_TO_RENDER_LIST_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ADD_TO_RENDER_LIST")
	{
		action = Trigger::_CreateAction(GraphicsScene::ADD_TO_RENDER_LIST_MESSAGE);
