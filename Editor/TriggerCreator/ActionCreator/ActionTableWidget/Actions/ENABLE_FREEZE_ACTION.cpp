...
void ActionTableWidget::ENABLE_FREEZE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ENABLE_FREEZE")
	{
...
bool ActionTableWidget::ENABLE_FREEZE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ENABLE_FREEZE") 
	{
...
QString ActionTableWidget::ENABLE_FREEZE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_FREEZE")
	{ 
		QString name;
...
bool ActionTableWidget::ENABLE_FREEZE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_FREEZE")
	{ 
...
bool ActionTableWidget::ENABLE_FREEZE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_FREEZE")
	{
...
void ActionTableWidget::ENABLE_FREEZE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_FREEZE")
	{
...
void ActionCreator::ENABLE_FREEZE_SetAction(int _actionType)
{
	if(_actionType == LogicObject::ENABLE_FREEZE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ENABLE_FREEZE");

		EnableFreezeAction* derived = dynamic_cast<EnableFreezeAction*>(action);
...
void ActionCreator::ENABLE_FREEZE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ENABLE_FREEZE")
	{
		action = Trigger::_CreateAction(LogicObject::ENABLE_FREEZE_MESSAGE);
