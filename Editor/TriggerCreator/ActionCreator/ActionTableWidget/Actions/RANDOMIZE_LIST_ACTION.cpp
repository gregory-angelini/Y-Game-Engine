...
void ActionTableWidget::RANDOMIZE_LIST_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "RANDOMIZE_LIST")
	{
...
bool ActionTableWidget::RANDOMIZE_LIST_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "RANDOMIZE_LIST") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Swaps"); 
...
QString ActionTableWidget::RANDOMIZE_LIST_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "RANDOMIZE_LIST")
	{ 
		QString name;
...
bool ActionTableWidget::RANDOMIZE_LIST_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "RANDOMIZE_LIST")
	{ 
...
bool ActionTableWidget::RANDOMIZE_LIST_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "RANDOMIZE_LIST")
	{
...
void ActionTableWidget::RANDOMIZE_LIST_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "RANDOMIZE_LIST")
	{
...
void ActionCreator::RANDOMIZE_LIST_SetAction(int _actionType)
{
	if(_actionType == AbstractList::RANDOMIZE_LIST_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "RANDOMIZE_LIST");

		RandomizeListAction* derived = dynamic_cast<RandomizeListAction*>(action);
...
void ActionCreator::RANDOMIZE_LIST_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "RANDOMIZE_LIST")
	{
		action = Trigger::_CreateAction(AbstractList::RANDOMIZE_LIST_MESSAGE);
