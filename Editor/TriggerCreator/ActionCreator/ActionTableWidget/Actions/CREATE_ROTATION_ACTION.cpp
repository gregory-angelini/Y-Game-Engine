...
void ActionTableWidget::CREATE_ROTATION_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_ROTATION")
	{
...
bool ActionTableWidget::CREATE_ROTATION_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_ROTATION") 
	{
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "Angle (in radians)"); 
...
QString ActionTableWidget::CREATE_ROTATION_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_ROTATION")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_ROTATION_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_ROTATION")
	{ 
...
bool ActionTableWidget::CREATE_ROTATION_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_ROTATION")
	{
...
bool ActionTableWidget::CREATE_ROTATION_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_ROTATION")
	{
...
void ActionTableWidget::CREATE_ROTATION_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_ROTATION")
	{
...
void ActionCreator::CREATE_ROTATION_SetAction(int _actionType)
{
	if(_actionType == RotatableObject::CREATE_ROTATION_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_ROTATION");

		CreateRotationAction* derived = dynamic_cast<CreateRotationAction*>(action);
...
void ActionCreator::CREATE_ROTATION_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_ROTATION")
	{
		action = Trigger::_CreateAction(RotatableObject::CREATE_ROTATION_MESSAGE);
