...
void ActionTableWidget::GET_LINE_LENGTH_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_LINE_LENGTH")
	{
...
bool ActionTableWidget::GET_LINE_LENGTH_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_LINE_LENGTH") 
	{
...
QString ActionTableWidget::GET_LINE_LENGTH_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_LINE_LENGTH")
	{ 
		QString name;
...
bool ActionTableWidget::GET_LINE_LENGTH_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_LINE_LENGTH")
	{
...
bool ActionTableWidget::GET_LINE_LENGTH_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_LINE_LENGTH")
	{
...
bool ActionTableWidget::GET_LINE_LENGTH_UpdateArg3VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_LINE_LENGTH")
	{
...
void ActionTableWidget::GET_LINE_LENGTH_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_LINE_LENGTH")
	{
...
void ActionCreator::GET_LINE_LENGTH_SetAction(int _actionType)
{
	if(_actionType == Line::GET_LINE_LENGTH_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_LINE_LENGTH");

		GetLineLengthAction* derived = dynamic_cast<GetLineLengthAction*>(action);
...
void ActionCreator::GET_LINE_LENGTH_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_LINE_LENGTH")
	{
		action = Trigger::_CreateAction(Line::GET_LINE_LENGTH_MESSAGE);
