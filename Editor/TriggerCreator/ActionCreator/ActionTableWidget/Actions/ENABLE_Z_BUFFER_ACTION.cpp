...
void ActionTableWidget::ENABLE_Z_BUFFER_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ENABLE_Z_BUFFER")
	{
...
bool ActionTableWidget::ENABLE_Z_BUFFER_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ENABLE_Z_BUFFER") 
	{
...
QString ActionTableWidget::ENABLE_Z_BUFFER_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_Z_BUFFER")
	{ 
		QString name;
...
bool ActionTableWidget::ENABLE_Z_BUFFER_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_Z_BUFFER")
	{
...
void ActionTableWidget::ENABLE_Z_BUFFER_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_Z_BUFFER")
	{
...
void ActionCreator::ENABLE_Z_BUFFER_SetAction(int _actionType)
{
	if(_actionType == Renderer::ENABLE_Z_BUFFER_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ENABLE_Z_BUFFER");

		EnableZBufferAction* derived = dynamic_cast<EnableZBufferAction*>(action);
...
void ActionCreator::ENABLE_Z_BUFFER_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ENABLE_Z_BUFFER")
	{
		action = Trigger::_CreateAction(Renderer::ENABLE_Z_BUFFER_MESSAGE);
