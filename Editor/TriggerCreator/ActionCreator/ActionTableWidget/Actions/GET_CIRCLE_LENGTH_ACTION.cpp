...
void ActionTableWidget::GET_CIRCLE_LENGTH_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_CIRCLE_LENGTH")
	{
...
bool ActionTableWidget::GET_CIRCLE_LENGTH_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_CIRCLE_LENGTH") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Radius"); 
...
QString ActionTableWidget::GET_CIRCLE_LENGTH_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CIRCLE_LENGTH")
	{ 
		QString name;
...
bool ActionTableWidget::GET_CIRCLE_LENGTH_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CIRCLE_LENGTH")
	{
...
bool ActionTableWidget::GET_CIRCLE_LENGTH_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CIRCLE_LENGTH")
	{
...
void ActionTableWidget::GET_CIRCLE_LENGTH_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CIRCLE_LENGTH")
	{
...
void ActionCreator::GET_CIRCLE_LENGTH_SetAction(int _actionType)
{
	if(_actionType == Circle::GET_CIRCLE_LENGTH_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_CIRCLE_LENGTH");

		GetCircleLengthAction* derived = dynamic_cast<GetCircleLengthAction*>(action);
...
void ActionCreator::GET_CIRCLE_LENGTH_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_CIRCLE_LENGTH")
	{
		action = Trigger::_CreateAction(Circle::GET_CIRCLE_LENGTH_MESSAGE);
