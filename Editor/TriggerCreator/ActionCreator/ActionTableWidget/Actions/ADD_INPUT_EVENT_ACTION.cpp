...
void ActionTableWidget::ADD_INPUT_EVENT_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ADD_INPUT_EVENT")
	{
...
bool ActionTableWidget::ADD_INPUT_EVENT_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ADD_INPUT_EVENT") 
	{
...
QString ActionTableWidget::ADD_INPUT_EVENT_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_INPUT_EVENT")
	{ 
		QString name;
...
bool ActionTableWidget::ADD_INPUT_EVENT_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_INPUT_EVENT")
	{ 
...
bool ActionTableWidget::ADD_INPUT_EVENT_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_INPUT_EVENT")
	{
...
bool ActionTableWidget::ADD_INPUT_EVENT_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_INPUT_EVENT")
	{
...
bool ActionTableWidget::ADD_INPUT_EVENT_UpdateArg3VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_INPUT_EVENT")
	{
...
void ActionTableWidget::ADD_INPUT_EVENT_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ADD_INPUT_EVENT")
	{
...
void ActionCreator::ADD_INPUT_EVENT_SetAction(int _actionType)
{
	if(_actionType == PlayerControl::ADD_INPUT_EVENT_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ADD_INPUT_EVENT");

		AddInputEventAction* derived = dynamic_cast<AddInputEventAction*>(action);
...
void ActionCreator::ADD_INPUT_EVENT_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ADD_INPUT_EVENT")
	{
		action = Trigger::_CreateAction(PlayerControl::ADD_INPUT_EVENT_MESSAGE);
