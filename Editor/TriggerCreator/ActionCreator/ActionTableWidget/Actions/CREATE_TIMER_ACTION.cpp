...
void ActionTableWidget::CREATE_TIMER_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_TIMER")
	{
...
bool ActionTableWidget::CREATE_TIMER_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_TIMER") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Timer name"); 
...
QString ActionTableWidget::CREATE_TIMER_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TIMER")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_TIMER_UpdateArg1KeyPropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TIMER")
	{ 
...
bool ActionTableWidget::CREATE_TIMER_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TIMER")
	{
...
void ActionTableWidget::CREATE_TIMER_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TIMER")
	{
...
void ActionCreator::CREATE_TIMER_SetAction(int _actionType)
{
	if(_actionType == Timer::CREATE_TIMER_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_TIMER");

		CreateTimerAction* derived = dynamic_cast<CreateTimerAction*>(action);
...
void ActionCreator::CREATE_TIMER_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_TIMER")
	{
		action = Trigger::_CreateAction(Timer::CREATE_TIMER_MESSAGE);
