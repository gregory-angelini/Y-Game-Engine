...
void ActionTableWidget::SEND_MESSAGE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "SEND_MESSAGE")
	{
...
bool ActionTableWidget::SEND_MESSAGE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				...
				SetCellText(ARG1_PROPERTY, DATA_TYPE, "String");
				    ...
				    item(ARG1_PROPERTY, ARG_MODE)->text() == "TEMPLATE")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Text"); 
...
QString ActionTableWidget::SEND_MESSAGE_CreateAutoName(void)
{
...
bool ActionTableWidget::SEND_MESSAGE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SEND_MESSAGE")
	{ 
...
bool ActionTableWidget::SEND_MESSAGE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SEND_MESSAGE")
	{
...
void ActionTableWidget::SEND_MESSAGE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SEND_MESSAGE")
	{
...
void ActionCreator::SEND_MESSAGE_SetAction(int _actionType)
{
	if(_actionType == AbstractObject::SEND_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "SEND_MESSAGE");

		SendMessageAction* derived = dynamic_cast<SendMessageAction*>(action);
...
void ActionCreator::SEND_MESSAGE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "SEND_MESSAGE")
	{
		action = Trigger::_CreateAction(AbstractObject::SEND_MESSAGE);
