...
void ActionTableWidget::CHANGE_ZOOM_NEAR_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_ZOOM_NEAR")
	{
...
bool ActionTableWidget::CHANGE_ZOOM_NEAR_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_ZOOM_NEAR") 
	{
...
QString ActionTableWidget::CHANGE_ZOOM_NEAR_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_NEAR")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_ZOOM_NEAR_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_NEAR")
	{ 
...
bool ActionTableWidget::CHANGE_ZOOM_NEAR_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_NEAR")
	{
...
void ActionTableWidget::CHANGE_ZOOM_NEAR_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_NEAR")
	{
...
void ActionCreator::CHANGE_ZOOM_NEAR_SetAction(int _actionType)
{
	if(_actionType == Camera3D::CHANGE_ZOOM_NEAR_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_ZOOM_NEAR");

		ChangeZoomNearAction* derived = dynamic_cast<ChangeZoomNearAction*>(action);
...
void ActionCreator::CHANGE_ZOOM_NEAR_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_ZOOM_NEAR")
	{
		action = Trigger::_CreateAction(Camera3D::CHANGE_ZOOM_NEAR_MESSAGE);
