...
void ActionTableWidget::GET_AMOUNT_OF_ITEMS_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_AMOUNT_OF_ITEMS")
	{
...
bool ActionTableWidget::GET_AMOUNT_OF_ITEMS_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_AMOUNT_OF_ITEMS") 
	{
...
QString ActionTableWidget::GET_AMOUNT_OF_ITEMS_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_AMOUNT_OF_ITEMS")
	{ 
		QString name;
...
bool ActionTableWidget::GET_AMOUNT_OF_ITEMS_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_AMOUNT_OF_ITEMS")
	{ 
...
bool ActionTableWidget::GET_AMOUNT_OF_ITEMS_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_AMOUNT_OF_ITEMS")
	{
...
void ActionTableWidget::GET_AMOUNT_OF_ITEMS_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_AMOUNT_OF_ITEMS")
	{
...
void ActionCreator::GET_AMOUNT_OF_ITEMS_SetAction(int _actionType)
{
	if(_actionType == AbstractList::GET_AMOUNT_OF_ITEMS_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_AMOUNT_OF_ITEMS");

		GetAmountOfItemsAction* derived = dynamic_cast<GetAmountOfItemsAction*>(action);
...
void ActionCreator::GET_AMOUNT_OF_ITEMS_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_AMOUNT_OF_ITEMS")
	{
		action = Trigger::_CreateAction(AbstractList::GET_AMOUNT_OF_ITEMS_MESSAGE);
