...
void ActionTableWidget::CREATE_BOX_TEXTURE_MAP_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_BOX_TEXTURE_MAP")
	{
...
bool ActionTableWidget::CREATE_BOX_TEXTURE_MAP_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_BOX_TEXTURE_MAP") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Texture size (in pixels)"); 
				SetCellText(ARG1_PROPERTY, DATA_TYPE, "Vector3");
			}
			else
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Texture size (in pixels)"); 
				SetCellText(ARG1_PROPERTY, DATA_TYPE, "Vector3 variable"); 
			}
		}
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "Texture color"); 
		...
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Texture"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::CREATE_BOX_TEXTURE_MAP_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_BOX_TEXTURE_MAP")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_BOX_TEXTURE_MAP_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_BOX_TEXTURE_MAP")
	{ 
		UpdateTexture(TARGET_PROPERTY);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::CREATE_BOX_TEXTURE_MAP_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_BOX_TEXTURE_MAP")
	{
		UpdateVariable(ARG1_PROPERTY, Variable<Vector3>::VECTOR_3_TYPE);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::CREATE_BOX_TEXTURE_MAP_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_BOX_TEXTURE_MAP")
	{
...
void ActionTableWidget::CREATE_BOX_TEXTURE_MAP_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_BOX_TEXTURE_MAP")
	{
...
void ActionCreator::CREATE_BOX_TEXTURE_MAP_SetAction(int _actionType)
{
	if(_actionType == Texture2D::CREATE_BOX_TEXTURE_MAP_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_BOX_TEXTURE_MAP");

		CreateBoxTextureMapAction* derived = dynamic_cast<CreateBoxTextureMapAction*>(action);
...
void ActionCreator::CREATE_BOX_TEXTURE_MAP_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_BOX_TEXTURE_MAP")
	{
		action = Trigger::_CreateAction(Texture2D::CREATE_BOX_TEXTURE_MAP_MESSAGE);
