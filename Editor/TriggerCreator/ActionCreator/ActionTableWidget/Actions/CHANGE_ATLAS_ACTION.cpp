...
void ActionTableWidget::CHANGE_ATLAS_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_ATLAS")
	{
		if(_index.column() == VALUE)
		{
			switch(_index.row())
			{
				case ARG1_PROPERTY:
				{
					...
					{
						_editor = UniversalDelegate::ATLAS_EDITOR;
						return;
					}
...
bool ActionTableWidget::CHANGE_ATLAS_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_ATLAS") 
	{
		...
		{ 
			...
			SetCellText(ARG1_PROPERTY, DATA_TYPE, "Atlas"); 
		}
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Textured animation"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::CHANGE_ATLAS_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ATLAS")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_ATLAS_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ATLAS")
	{ 
		UpdateTexturedAnimation(TARGET_PROPERTY);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::CHANGE_ATLAS_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ATLAS")
	{
		UpdateAtlas(ARG1_PROPERTY);
		return true;
	}
	return false;
}
...
void ActionTableWidget::CHANGE_ATLAS_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ATLAS")
	{
		if(item(_index.row(), ARG_MODE)->text() == "VARIABLE")
		{
			...
			{
				AtlasDropEvent(_it, _index, _suffix, _fileName);
			}
...
void ActionCreator::CHANGE_ATLAS_SetAction(int _actionType)
{
	if(_actionType == TexturedAnimation::CHANGE_ATLAS_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_ATLAS");

		ChangeAtlasAction* derived = dynamic_cast<ChangeAtlasAction*>(action);
...
void ActionCreator::CHANGE_ATLAS_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_ATLAS")
	{
		action = Trigger::_CreateAction(TexturedAnimation::CHANGE_ATLAS_MESSAGE);
