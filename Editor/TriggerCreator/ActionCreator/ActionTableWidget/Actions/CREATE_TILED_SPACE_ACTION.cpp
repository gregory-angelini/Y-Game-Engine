...
void ActionTableWidget::CREATE_TILED_SPACE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_TILED_SPACE")
	{
...
bool ActionTableWidget::CREATE_TILED_SPACE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_TILED_SPACE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Tiled space name"); 
...
QString ActionTableWidget::CREATE_TILED_SPACE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TILED_SPACE")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_TILED_SPACE_UpdateArg1KeyPropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TILED_SPACE")
	{ 
...
bool ActionTableWidget::CREATE_TILED_SPACE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TILED_SPACE")
	{
...
void ActionTableWidget::CREATE_TILED_SPACE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TILED_SPACE")
	{
...
void ActionCreator::CREATE_TILED_SPACE_SetAction(int _actionType)
{
	if(_actionType == TiledSpace3D::CREATE_TILED_SPACE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_TILED_SPACE");

		CreateTiledSpaceAction* derived = dynamic_cast<CreateTiledSpaceAction*>(action);
...
void ActionCreator::CREATE_TILED_SPACE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_TILED_SPACE")
	{
		action = Trigger::_CreateAction(TiledSpace3D::CREATE_TILED_SPACE_MESSAGE);
