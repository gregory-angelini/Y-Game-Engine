...
void ActionTableWidget::CHANGE_INTERSECTION_MODE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_INTERSECTION_MODE")
	{
...
bool ActionTableWidget::CHANGE_INTERSECTION_MODE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_INTERSECTION_MODE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Intersection mode"); 
...
QString ActionTableWidget::CHANGE_INTERSECTION_MODE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_INTERSECTION_MODE")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_INTERSECTION_MODE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_INTERSECTION_MODE")
	{ 
...
bool ActionTableWidget::CHANGE_INTERSECTION_MODE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_INTERSECTION_MODE")
	{
...
bool ActionTableWidget::CHANGE_INTERSECTION_MODE_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_INTERSECTION_MODE")
	{
...
void ActionTableWidget::CHANGE_INTERSECTION_MODE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_INTERSECTION_MODE")
	{
...
void ActionCreator::CHANGE_INTERSECTION_MODE_SetAction(int _actionType)
{
	if(_actionType == AbstractSurface::CHANGE_INTERSECTION_MODE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_INTERSECTION_MODE");

		ChangeIntersectionModeAction* derived = dynamic_cast<ChangeIntersectionModeAction*>(action);
...
void ActionCreator::CHANGE_INTERSECTION_MODE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_INTERSECTION_MODE")
	{
		action = Trigger::_CreateAction(AbstractSurface::CHANGE_INTERSECTION_MODE_MESSAGE);
