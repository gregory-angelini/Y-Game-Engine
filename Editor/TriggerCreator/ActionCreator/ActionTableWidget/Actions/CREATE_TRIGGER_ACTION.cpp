...
void ActionTableWidget::CREATE_TRIGGER_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_TRIGGER")
	{
		if(_index.column() == VALUE)
		{
			switch(_index.row())
			{
				...
				case ARG3_PROPERTY:
				{
					...
					{
						_editor = UniversalDelegate::ENUMERATION_EXPR_EDITOR;
						return;
					}
...
bool ActionTableWidget::CREATE_TRIGGER_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_TRIGGER") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Trigger name"); 
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "IN (Boolean expression)"); 
		...
		{ 
			...
			if(item(ARG3_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG3_PROPERTY, CONTENT, "OUT (Enumeration expression)"); 
...
QString ActionTableWidget::CREATE_TRIGGER_CreateAutoName(void)
{
...
bool ActionTableWidget::CREATE_TRIGGER_UpdateArg1KeyPropertyStatus(void)
{ 
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TRIGGER")
	{ 
...
bool ActionTableWidget::CREATE_TRIGGER_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TRIGGER")
	{
...
bool ActionTableWidget::CREATE_TRIGGER_UpdateArg2KeyPropertyStatus(void)
{ 
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TRIGGER")
	{
...
bool ActionTableWidget::CREATE_TRIGGER_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TRIGGER")
	{
...
bool ActionTableWidget::CREATE_TRIGGER_UpdateArg3KeyPropertyStatus(void)
{ 
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TRIGGER")
	{
		UpdateEnumerationExpr(ARG3_PROPERTY);
		return true;
	} 
	return false;
}
...
bool ActionTableWidget::CREATE_TRIGGER_UpdateArg3VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TRIGGER")
	{
...
void ActionTableWidget::CREATE_TRIGGER_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TRIGGER")
	{
		if(item(_index.row(), ARG_MODE)->text() == "KEY")
		{
			...
			{
				EnumerationExprDropEvent(_it, _index, _suffix, _fileName);
			}
		}
...
void ActionCreator::CREATE_TRIGGER_SetAction(int _actionType)
{
	if(_actionType == Trigger::CREATE_TRIGGER_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_TRIGGER");

		CreateTriggerAction* derived = dynamic_cast<CreateTriggerAction*>(action);
		...
		if(derived)
		{
			...
			if(derived->GetArgState(AbstractAction::KEY, AbstractAction::ARG2))
			{
				...
				actionTable->SetCellText(ActionTableWidget::ARG2_PROPERTY, ActionTableWidget::VALUE, derived->GetStringKeyArg(AbstractAction::ARG2).c_str());
			}
			...
			if(derived->GetArgState(AbstractAction::KEY, AbstractAction::ARG3))
			{
				...
				actionTable->SetCellText(ActionTableWidget::ARG3_PROPERTY, ActionTableWidget::VALUE, derived->GetStringKeyArg(AbstractAction::ARG3).c_str());
			}
...
void ActionCreator::CREATE_TRIGGER_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_TRIGGER")
	{
		action = Trigger::_CreateAction(Trigger::CREATE_TRIGGER_MESSAGE);
		...
		if(action)
		{
			...
			{
				action->SetStringKeyArg(AbstractAction::ARG2, actionTable->GetCellText(ActionTableWidget::ARG2_PROPERTY, ActionTableWidget::VALUE).toStdString());
			}
			...
			{
				action->SetStringKeyArg(AbstractAction::ARG3, actionTable->GetCellText(ActionTableWidget::ARG3_PROPERTY, ActionTableWidget::VALUE).toStdString());
			}
