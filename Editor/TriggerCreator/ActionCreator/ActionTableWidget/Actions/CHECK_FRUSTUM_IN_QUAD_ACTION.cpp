...
void ActionTableWidget::CHECK_FRUSTUM_IN_QUAD_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHECK_FRUSTUM_IN_QUAD")
	{
...
bool ActionTableWidget::CHECK_FRUSTUM_IN_QUAD_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHECK_FRUSTUM_IN_QUAD") 
	{
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "Center"); 
		...
		{ 
			...
			if(item(ARG3_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG3_PROPERTY, CONTENT, "Size"); 
...
QString ActionTableWidget::CHECK_FRUSTUM_IN_QUAD_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_FRUSTUM_IN_QUAD")
	{ 
		QString name;
...
bool ActionTableWidget::CHECK_FRUSTUM_IN_QUAD_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_FRUSTUM_IN_QUAD")
	{ 
...
bool ActionTableWidget::CHECK_FRUSTUM_IN_QUAD_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_FRUSTUM_IN_QUAD")
	{
...
bool ActionTableWidget::CHECK_FRUSTUM_IN_QUAD_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_FRUSTUM_IN_QUAD")
	{
...
bool ActionTableWidget::CHECK_FRUSTUM_IN_QUAD_UpdateArg3VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_FRUSTUM_IN_QUAD")
	{
...
void ActionTableWidget::CHECK_FRUSTUM_IN_QUAD_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_FRUSTUM_IN_QUAD")
	{
...
void ActionCreator::CHECK_FRUSTUM_IN_QUAD_SetAction(int _actionType)
{
	if(_actionType == Camera3D::CHECK_FRUSTUM_IN_QUAD_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHECK_FRUSTUM_IN_QUAD");

		CheckFrustumInQuadAction* derived = dynamic_cast<CheckFrustumInQuadAction*>(action);
		...
		if(derived)
		{
			...
			if(derived->GetArgState(AbstractAction::KEY, AbstractAction::ARG3))
			{
				...
				actionTable->SetCellText(ActionTableWidget::ARG3_PROPERTY, ActionTableWidget::VALUE, Vector2ToString(derived->GetVector2KeyArg(AbstractAction::ARG3)));
			}
...
void ActionCreator::CHECK_FRUSTUM_IN_QUAD_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHECK_FRUSTUM_IN_QUAD")
	{
		action = Trigger::_CreateAction(Camera3D::CHECK_FRUSTUM_IN_QUAD_MESSAGE);
		...
		if(action)
		{
			...
			{
				action->SetVector2KeyArg(AbstractAction::ARG3, StringToVector2(actionTable->GetCellText(ActionTableWidget::ARG3_PROPERTY, ActionTableWidget::VALUE)));
			}
