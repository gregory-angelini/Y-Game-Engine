...
void ActionTableWidget::CHANGE_DISTANCE_TO_VIEWING_POINT_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_DISTANCE_TO_VIEWING_POINT")
	{
...
bool ActionTableWidget::CHANGE_DISTANCE_TO_VIEWING_POINT_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_DISTANCE_TO_VIEWING_POINT") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Distance"); 
...
QString ActionTableWidget::CHANGE_DISTANCE_TO_VIEWING_POINT_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DISTANCE_TO_VIEWING_POINT")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_DISTANCE_TO_VIEWING_POINT_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DISTANCE_TO_VIEWING_POINT")
	{ 
...
bool ActionTableWidget::CHANGE_DISTANCE_TO_VIEWING_POINT_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DISTANCE_TO_VIEWING_POINT")
	{
...
void ActionTableWidget::CHANGE_DISTANCE_TO_VIEWING_POINT_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_DISTANCE_TO_VIEWING_POINT")
	{
...
void ActionCreator::CHANGE_DISTANCE_TO_VIEWING_POINT_SetAction(int _actionType)
{
	if(_actionType == Camera3D::CHANGE_DISTANCE_TO_VIEWING_POINT_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_DISTANCE_TO_VIEWING_POINT");

		ChangeDistanceToViewingPointAction* derived = dynamic_cast<ChangeDistanceToViewingPointAction*>(action);
...
void ActionCreator::CHANGE_DISTANCE_TO_VIEWING_POINT_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_DISTANCE_TO_VIEWING_POINT")
	{
		action = Trigger::_CreateAction(Camera3D::CHANGE_DISTANCE_TO_VIEWING_POINT_MESSAGE);
