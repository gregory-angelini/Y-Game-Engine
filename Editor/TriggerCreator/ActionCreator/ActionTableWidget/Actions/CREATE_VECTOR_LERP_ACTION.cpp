...
void ActionTableWidget::CREATE_VECTOR_LERP_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_VECTOR_LERP")
	{
...
bool ActionTableWidget::CREATE_VECTOR_LERP_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_VECTOR_LERP") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Vector lerp name"); 
...
QString ActionTableWidget::CREATE_VECTOR_LERP_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_VECTOR_LERP")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_VECTOR_LERP_UpdateArg1KeyPropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_VECTOR_LERP")
	{ 
...
bool ActionTableWidget::CREATE_VECTOR_LERP_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_VECTOR_LERP")
	{
...
void ActionTableWidget::CREATE_VECTOR_LERP_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_VECTOR_LERP")
	{
...
void ActionCreator::CREATE_VECTOR_LERP_SetAction(int _actionType)
{
	if(_actionType == VectorLerp::CREATE_VECTOR_LERP_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_VECTOR_LERP");

		CreateVectorLerpAction* derived = dynamic_cast<CreateVectorLerpAction*>(action);
...
void ActionCreator::CREATE_VECTOR_LERP_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_VECTOR_LERP")
	{
		action = Trigger::_CreateAction(VectorLerp::CREATE_VECTOR_LERP_MESSAGE);
