...
void ActionTableWidget::GET_ITEM_INDEX_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_ITEM_INDEX")
	{
...
bool ActionTableWidget::GET_ITEM_INDEX_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_ITEM_INDEX") 
	{
		...
		else if(_index.row() == ARG1_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(ARG1_PROPERTY, CONTENT, "Item"); 
...
QString ActionTableWidget::GET_ITEM_INDEX_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_ITEM_INDEX")
	{ 
		QString name;
...
bool ActionTableWidget::GET_ITEM_INDEX_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_ITEM_INDEX")
	{ 
...
bool ActionTableWidget::GET_ITEM_INDEX_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_ITEM_INDEX")
	{
...
bool ActionTableWidget::GET_ITEM_INDEX_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_ITEM_INDEX")
	{
...
void ActionTableWidget::GET_ITEM_INDEX_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_ITEM_INDEX")
	{
...
void ActionCreator::GET_ITEM_INDEX_SetAction(int _actionType)
{
	if(_actionType == AbstractList::GET_ITEM_INDEX_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_ITEM_INDEX");

		GetItemIndexAction* derived = dynamic_cast<GetItemIndexAction*>(action);
...
void ActionCreator::GET_ITEM_INDEX_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_ITEM_INDEX")
	{
		action = Trigger::_CreateAction(AbstractList::GET_ITEM_INDEX_MESSAGE);
