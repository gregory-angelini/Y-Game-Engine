...
void ActionTableWidget::SWAP_TWO_TILES_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "SWAP_TWO_TILES")
	{
...
bool ActionTableWidget::SWAP_TWO_TILES_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "SWAP_TWO_TILES") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Tile1"); 
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "Tile2"); 
		...
		{ 
			...
			SetCellText(ARG3_PROPERTY, CONTENT, "Swap result"); 
...
QString ActionTableWidget::SWAP_TWO_TILES_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SWAP_TWO_TILES")
	{ 
		QString name;
...
bool ActionTableWidget::SWAP_TWO_TILES_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SWAP_TWO_TILES")
	{ 
...
bool ActionTableWidget::SWAP_TWO_TILES_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SWAP_TWO_TILES")
	{
...
bool ActionTableWidget::SWAP_TWO_TILES_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SWAP_TWO_TILES")
	{ 
...
bool ActionTableWidget::SWAP_TWO_TILES_UpdateArg3VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SWAP_TWO_TILES")
	{
...
void ActionTableWidget::SWAP_TWO_TILES_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "SWAP_TWO_TILES")
	{
...
void ActionCreator::SWAP_TWO_TILES_SetAction(int _actionType)
{
	if(_actionType == TiledSpace3D::SWAP_TWO_TILES_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "SWAP_TWO_TILES");

		SwapTwoTilesAction* derived = dynamic_cast<SwapTwoTilesAction*>(action);
...
void ActionCreator::SWAP_TWO_TILES_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "SWAP_TWO_TILES")
	{
		action = Trigger::_CreateAction(TiledSpace3D::SWAP_TWO_TILES_MESSAGE);
