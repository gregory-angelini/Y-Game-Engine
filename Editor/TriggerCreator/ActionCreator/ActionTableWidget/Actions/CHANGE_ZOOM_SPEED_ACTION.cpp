...
void ActionTableWidget::CHANGE_ZOOM_SPEED_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_ZOOM_SPEED")
	{
...
bool ActionTableWidget::CHANGE_ZOOM_SPEED_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_ZOOM_SPEED") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Zoom speed"); 
...
QString ActionTableWidget::CHANGE_ZOOM_SPEED_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_SPEED")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_ZOOM_SPEED_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_SPEED")
	{ 
...
bool ActionTableWidget::CHANGE_ZOOM_SPEED_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_SPEED")
	{
...
void ActionTableWidget::CHANGE_ZOOM_SPEED_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_SPEED")
	{
...
void ActionCreator::CHANGE_ZOOM_SPEED_SetAction(int _actionType)
{
	if(_actionType == Camera3D::CHANGE_ZOOM_SPEED_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_ZOOM_SPEED");

		ChangeZoomSpeedAction* derived = dynamic_cast<ChangeZoomSpeedAction*>(action);
...
void ActionCreator::CHANGE_ZOOM_SPEED_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_ZOOM_SPEED")
	{
		action = Trigger::_CreateAction(Camera3D::CHANGE_ZOOM_SPEED_MESSAGE);
