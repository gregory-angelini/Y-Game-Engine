...
void ActionTableWidget::CHANGE_FLOAT_LERP_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_FLOAT_LERP")
	{
		if(_index.column() == VALUE)
		{
			switch(_index.row())
			{
				...
				case TARGET_PROPERTY:
				{
					...
					{
						_editor = UniversalDelegate::LERPING_OBJECT_EDITOR;
						return;
					}
...
bool ActionTableWidget::CHANGE_FLOAT_LERP_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_FLOAT_LERP") 
	{
		...
		{ 
			...
			SetCellText(ARG1_PROPERTY, DATA_TYPE, "Float lerp"); 
		}
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Lerping object"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::CHANGE_FLOAT_LERP_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_FLOAT_LERP")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_FLOAT_LERP_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_FLOAT_LERP")
	{ 
		UpdateLerpingObject(TARGET_PROPERTY);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::CHANGE_FLOAT_LERP_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_FLOAT_LERP")
	{
		UpdateFloatLerp(ARG1_PROPERTY);
		return true;
	}
	return false;
}
...
void ActionTableWidget::CHANGE_FLOAT_LERP_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_FLOAT_LERP")
	{
		if(item(_index.row(), ARG_MODE)->text() == "VARIABLE")
		{
			...
			{
				LerpingObjectDropEvent(_it, _index, _suffix, _fileName);
			}
		}
...
void ActionCreator::CHANGE_FLOAT_LERP_SetAction(int _actionType)
{
	if(_actionType == LerpingObject::CHANGE_FLOAT_LERP_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_FLOAT_LERP");

		ChangeFloatLerpAction* derived = dynamic_cast<ChangeFloatLerpAction*>(action);
...
void ActionCreator::CHANGE_FLOAT_LERP_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_FLOAT_LERP")
	{
		action = Trigger::_CreateAction(LerpingObject::CHANGE_FLOAT_LERP_MESSAGE);
