...
void ActionTableWidget::CHANGE_ROTATION_MODE_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_ROTATION_MODE")
	{
...
bool ActionTableWidget::CHANGE_ROTATION_MODE_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_ROTATION_MODE") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Rotation mode"); 
...
QString ActionTableWidget::CHANGE_ROTATION_MODE_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ROTATION_MODE")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_ROTATION_MODE_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ROTATION_MODE")
	{ 
...
bool ActionTableWidget::CHANGE_ROTATION_MODE_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ROTATION_MODE")
	{
...
void ActionTableWidget::CHANGE_ROTATION_MODE_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ROTATION_MODE")
	{
...
void ActionCreator::CHANGE_ROTATION_MODE_SetAction(int _actionType)
{
	if(_actionType == Camera3D::CHANGE_ROTATION_MODE_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_ROTATION_MODE");

		ChangeRotationModeAction* derived = dynamic_cast<ChangeRotationModeAction*>(action);
...
void ActionCreator::CHANGE_ROTATION_MODE_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_ROTATION_MODE")
	{
		action = Trigger::_CreateAction(Camera3D::CHANGE_ROTATION_MODE_MESSAGE);
