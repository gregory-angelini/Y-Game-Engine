...
void ActionTableWidget::ROTATE_LIST_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ROTATE_LIST")
	{
...
bool ActionTableWidget::ROTATE_LIST_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ROTATE_LIST") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "ToRight/ToLeft"); 
...
QString ActionTableWidget::ROTATE_LIST_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ROTATE_LIST")
	{ 
		QString name;
...
bool ActionTableWidget::ROTATE_LIST_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ROTATE_LIST")
	{ 
...
bool ActionTableWidget::ROTATE_LIST_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ROTATE_LIST")
	{
...
void ActionTableWidget::ROTATE_LIST_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ROTATE_LIST")
	{
...
void ActionCreator::ROTATE_LIST_SetAction(int _actionType)
{
	if(_actionType == AbstractList::ROTATE_LIST_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ROTATE_LIST");

		RotateListAction* derived = dynamic_cast<RotateListAction*>(action);
...
void ActionCreator::ROTATE_LIST_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ROTATE_LIST")
	{
		action = Trigger::_CreateAction(AbstractList::ROTATE_LIST_MESSAGE);
