...
void ActionTableWidget::GET_COLOR_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_COLOR")
	{
...
bool ActionTableWidget::GET_COLOR_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_COLOR") 
	{
...
QString ActionTableWidget::GET_COLOR_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_COLOR")
	{ 
		QString name;
...
bool ActionTableWidget::GET_COLOR_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_COLOR")
	{ 
...
bool ActionTableWidget::GET_COLOR_UpdateArg1PropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_COLOR")
	{ 
...
bool ActionTableWidget::GET_COLOR_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_COLOR")
	{
		if(item(ARG1_PROPERTY, VALUE)->text() == "RGB_COLOR")
...
void ActionTableWidget::GET_COLOR_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_COLOR")
	{
...
void ActionCreator::GET_COLOR_SetAction(int _actionType)
{
	if(_actionType == ColorableObject::GET_COLOR_HSL_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_COLOR");

		GetColorHSLAction* derived = dynamic_cast<GetColorHSLAction*>(action);
	...
	else if(_actionType == ColorableObject::GET_COLOR_RGB_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_COLOR");

		GetColorRGBAction* derived = dynamic_cast<GetColorRGBAction*>(action);
...
void ActionCreator::GET_COLOR_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_COLOR")
	{
		if(actionTable->GetCellText(ActionTableWidget::ARG1_PROPERTY, ActionTableWidget::VALUE) == "HSL_COLOR")
		{
			action = Trigger::_CreateAction(ColorableObject::GET_COLOR_HSL_MESSAGE);
		...
		{  
			action = Trigger::_CreateAction(ColorableObject::GET_COLOR_RGB_MESSAGE);
