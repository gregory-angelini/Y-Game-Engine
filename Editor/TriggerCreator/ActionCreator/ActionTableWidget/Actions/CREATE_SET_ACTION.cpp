...
void ActionTableWidget::CREATE_SET_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_SET")
	{
...
bool ActionTableWidget::CREATE_SET_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_SET") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Set name"); 
...
QString ActionTableWidget::CREATE_SET_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_SET")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_SET_UpdateArg1KeyPropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_SET")
	{ 
...
bool ActionTableWidget::CREATE_SET_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_SET")
	{
...
void ActionTableWidget::CREATE_SET_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_SET")
	{
...
void ActionCreator::CREATE_SET_SetAction(int _actionType)
{
	if(_actionType == Set::CREATE_SET_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_SET");

		CreateSetAction* derived = dynamic_cast<CreateSetAction*>(action);
...
void ActionCreator::CREATE_SET_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_SET")
	{
		action = Trigger::_CreateAction(Set::CREATE_SET_MESSAGE);
