...
void ActionTableWidget::GET_NAME_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_NAME")
	{
...
bool ActionTableWidget::GET_NAME_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_NAME") 
	{
...
QString ActionTableWidget::GET_NAME_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_NAME")
	{ 
		QString name;
...
bool ActionTableWidget::GET_NAME_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_NAME")
	{ 
...
bool ActionTableWidget::GET_NAME_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_NAME")
	{
...
void ActionTableWidget::GET_NAME_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_NAME")
	{
...
void ActionCreator::GET_NAME_SetAction(int _actionType)
{
	if(_actionType == AbstractObject::GET_NAME_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_NAME");

		GetNameAction* derived = dynamic_cast<GetNameAction*>(action);
...
void ActionCreator::GET_NAME_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_NAME")
	{
		action = Trigger::_CreateAction(AbstractObject::GET_NAME_MESSAGE);
