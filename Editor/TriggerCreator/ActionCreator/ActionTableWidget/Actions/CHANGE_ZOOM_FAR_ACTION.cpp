...
void ActionTableWidget::CHANGE_ZOOM_FAR_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_ZOOM_FAR")
	{
...
bool ActionTableWidget::CHANGE_ZOOM_FAR_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_ZOOM_FAR") 
	{
...
QString ActionTableWidget::CHANGE_ZOOM_FAR_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_FAR")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_ZOOM_FAR_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_FAR")
	{ 
...
bool ActionTableWidget::CHANGE_ZOOM_FAR_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_FAR")
	{
...
void ActionTableWidget::CHANGE_ZOOM_FAR_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ZOOM_FAR")
	{
...
void ActionCreator::CHANGE_ZOOM_FAR_SetAction(int _actionType)
{
	if(_actionType == Camera3D::CHANGE_ZOOM_FAR_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_ZOOM_FAR");

		ChangeZoomFarAction* derived = dynamic_cast<ChangeZoomFarAction*>(action);
...
void ActionCreator::CHANGE_ZOOM_FAR_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_ZOOM_FAR")
	{
		action = Trigger::_CreateAction(Camera3D::CHANGE_ZOOM_FAR_MESSAGE);
