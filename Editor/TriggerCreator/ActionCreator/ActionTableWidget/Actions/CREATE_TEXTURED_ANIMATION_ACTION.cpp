...
void ActionTableWidget::CREATE_TEXTURED_ANIMATION_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CREATE_TEXTURED_ANIMATION")
	{
...
bool ActionTableWidget::CREATE_TEXTURED_ANIMATION_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CREATE_TEXTURED_ANIMATION") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Textured animation name"); 
...
QString ActionTableWidget::CREATE_TEXTURED_ANIMATION_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TEXTURED_ANIMATION")
	{ 
		QString name;
...
bool ActionTableWidget::CREATE_TEXTURED_ANIMATION_UpdateArg1KeyPropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TEXTURED_ANIMATION")
	{ 
...
bool ActionTableWidget::CREATE_TEXTURED_ANIMATION_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TEXTURED_ANIMATION")
	{
...
void ActionTableWidget::CREATE_TEXTURED_ANIMATION_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CREATE_TEXTURED_ANIMATION")
	{
...
void ActionCreator::CREATE_TEXTURED_ANIMATION_SetAction(int _actionType)
{
	if(_actionType == TexturedAnimation::CREATE_TEXTURED_ANIMATION_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CREATE_TEXTURED_ANIMATION");

		CreateTexturedAnimationAction* derived = dynamic_cast<CreateTexturedAnimationAction*>(action);
...
void ActionCreator::CREATE_TEXTURED_ANIMATION_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CREATE_TEXTURED_ANIMATION")
	{
		action = Trigger::_CreateAction(TexturedAnimation::CREATE_TEXTURED_ANIMATION_MESSAGE);
