...
void ActionTableWidget::CHANGE_ACTIVATION_LIMIT_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHANGE_ACTIVATION_LIMIT")
	{
...
bool ActionTableWidget::CHANGE_ACTIVATION_LIMIT_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_ACTIVATION_LIMIT") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Activation limit"); 
...
QString ActionTableWidget::CHANGE_ACTIVATION_LIMIT_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ACTIVATION_LIMIT")
	{ 
		QString name;
...
bool ActionTableWidget::CHANGE_ACTIVATION_LIMIT_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ACTIVATION_LIMIT")
	{ 
...
bool ActionTableWidget::CHANGE_ACTIVATION_LIMIT_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ACTIVATION_LIMIT")
	{
...
void ActionTableWidget::CHANGE_ACTIVATION_LIMIT_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_ACTIVATION_LIMIT")
	{
...
void ActionCreator::CHANGE_ACTIVATION_LIMIT_SetAction(int _actionType)
{
	if(_actionType == ActivatableObject::CHANGE_ACTIVATION_LIMIT_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHANGE_ACTIVATION_LIMIT");

		ChangeActivationLimitAction* derived = dynamic_cast<ChangeActivationLimitAction*>(action);
...
void ActionCreator::CHANGE_ACTIVATION_LIMIT_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHANGE_ACTIVATION_LIMIT")
	{
		action = Trigger::_CreateAction(ActivatableObject::CHANGE_ACTIVATION_LIMIT_MESSAGE);
