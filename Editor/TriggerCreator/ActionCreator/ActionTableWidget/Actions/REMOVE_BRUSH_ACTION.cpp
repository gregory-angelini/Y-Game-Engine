...
void ActionTableWidget::REMOVE_BRUSH_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "REMOVE_BRUSH")
	{
...
bool ActionTableWidget::REMOVE_BRUSH_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "REMOVE_BRUSH") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Brush id"); 
...
QString ActionTableWidget::REMOVE_BRUSH_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REMOVE_BRUSH")
	{ 
		QString name;
...
bool ActionTableWidget::REMOVE_BRUSH_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REMOVE_BRUSH")
	{ 
...
bool ActionTableWidget::REMOVE_BRUSH_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REMOVE_BRUSH")
	{
...
void ActionTableWidget::REMOVE_BRUSH_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REMOVE_BRUSH")
	{
...
void ActionCreator::REMOVE_BRUSH_SetAction(int _actionType)
{
	if(_actionType == TiledSpace3D::REMOVE_BRUSH_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "REMOVE_BRUSH");

		RemoveBrushAction* derived = dynamic_cast<RemoveBrushAction*>(action);
...
void ActionCreator::REMOVE_BRUSH_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "REMOVE_BRUSH")
	{
		action = Trigger::_CreateAction(TiledSpace3D::REMOVE_BRUSH_MESSAGE);
