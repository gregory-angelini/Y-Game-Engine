...
void ActionTableWidget::GET_LIST_LENGTH_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_LIST_LENGTH")
	{
...
bool ActionTableWidget::GET_LIST_LENGTH_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_LIST_LENGTH") 
	{
...
QString ActionTableWidget::GET_LIST_LENGTH_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_LIST_LENGTH")
	{ 
		QString name;
...
bool ActionTableWidget::GET_LIST_LENGTH_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_LIST_LENGTH")
	{ 
...
bool ActionTableWidget::GET_LIST_LENGTH_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_LIST_LENGTH")
	{
...
void ActionTableWidget::GET_LIST_LENGTH_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_LIST_LENGTH")
	{
...
void ActionCreator::GET_LIST_LENGTH_SetAction(int _actionType)
{
	if(_actionType == AbstractList::GET_LIST_LENGTH_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_LIST_LENGTH");

		GetListLengthAction* derived = dynamic_cast<GetListLengthAction*>(action);
...
void ActionCreator::GET_LIST_LENGTH_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_LIST_LENGTH")
	{
		action = Trigger::_CreateAction(AbstractList::GET_LIST_LENGTH_MESSAGE);
