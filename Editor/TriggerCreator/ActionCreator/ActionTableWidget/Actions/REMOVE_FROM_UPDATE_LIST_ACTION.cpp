...
void ActionTableWidget::REMOVE_FROM_UPDATE_LIST_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "REMOVE_FROM_UPDATE_LIST")
	{
...
bool ActionTableWidget::REMOVE_FROM_UPDATE_LIST_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "REMOVE_FROM_UPDATE_LIST") 
	{
		...
		{ 
			...
			SetCellText(ARG1_PROPERTY, DATA_TYPE, "Logic object"); 
		}
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Logic scene"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::REMOVE_FROM_UPDATE_LIST_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REMOVE_FROM_UPDATE_LIST")
	{ 
		QString name;
...
bool ActionTableWidget::REMOVE_FROM_UPDATE_LIST_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REMOVE_FROM_UPDATE_LIST")
	{ 
...
bool ActionTableWidget::REMOVE_FROM_UPDATE_LIST_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REMOVE_FROM_UPDATE_LIST")
	{
		UpdateLogicObject(ARG1_PROPERTY); 
		return true;
	}
	return false;
}
...
void ActionTableWidget::REMOVE_FROM_UPDATE_LIST_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REMOVE_FROM_UPDATE_LIST")
	{
...
void ActionCreator::REMOVE_FROM_UPDATE_LIST_SetAction(int _actionType)
{
	if(_actionType == LogicScene::REMOVE_FROM_UPDATE_LIST_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "REMOVE_FROM_UPDATE_LIST");

		RemoveFromUpdateListAction* derived = dynamic_cast<RemoveFromUpdateListAction*>(action);
...
void ActionCreator::REMOVE_FROM_UPDATE_LIST_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "REMOVE_FROM_UPDATE_LIST")
	{
		action = Trigger::_CreateAction(LogicScene::REMOVE_FROM_UPDATE_LIST_MESSAGE);
