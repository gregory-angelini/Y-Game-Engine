...
void ActionTableWidget::STREAMING_FLOAT_CURRENT_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "STREAMING_FLOAT_CURRENT")
	{
...
bool ActionTableWidget::STREAMING_FLOAT_CURRENT_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "STREAMING_FLOAT_CURRENT") 
	{
...
QString ActionTableWidget::STREAMING_FLOAT_CURRENT_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "STREAMING_FLOAT_CURRENT")
	{ 
		QString name;
...
bool ActionTableWidget::STREAMING_FLOAT_CURRENT_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "STREAMING_FLOAT_CURRENT")
	{ 
...
bool ActionTableWidget::STREAMING_FLOAT_CURRENT_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "STREAMING_FLOAT_CURRENT")
	{
...
bool ActionTableWidget::STREAMING_FLOAT_CURRENT_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "STREAMING_FLOAT_CURRENT")
	{
...
void ActionTableWidget::STREAMING_FLOAT_CURRENT_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "STREAMING_FLOAT_CURRENT")
	{
...
void ActionCreator::STREAMING_FLOAT_CURRENT_SetAction(int _actionType)
{
	if(_actionType == FloatLerp::STREAMING_FLOAT_CURRENT_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "STREAMING_FLOAT_CURRENT");

		StreamingFloatCurrentAction* derived = dynamic_cast<StreamingFloatCurrentAction*>(action);
...
void ActionCreator::STREAMING_FLOAT_CURRENT_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "STREAMING_FLOAT_CURRENT")
	{
		action = Trigger::_CreateAction(FloatLerp::STREAMING_FLOAT_CURRENT_MESSAGE);
