...
void ActionTableWidget::GET_CONTROL_QUATERNIONS_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_CONTROL_QUATERNIONS")
	{
...
bool ActionTableWidget::GET_CONTROL_QUATERNIONS_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_CONTROL_QUATERNIONS") 
	{
...
QString ActionTableWidget::GET_CONTROL_QUATERNIONS_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CONTROL_QUATERNIONS")
	{ 
		QString name;
...
bool ActionTableWidget::GET_CONTROL_QUATERNIONS_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CONTROL_QUATERNIONS")
	{ 
...
bool ActionTableWidget::GET_CONTROL_QUATERNIONS_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CONTROL_QUATERNIONS")
	{
...
bool ActionTableWidget::GET_CONTROL_QUATERNIONS_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CONTROL_QUATERNIONS")
	{
...
void ActionTableWidget::GET_CONTROL_QUATERNIONS_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_CONTROL_QUATERNIONS")
	{
...
void ActionCreator::GET_CONTROL_QUATERNIONS_SetAction(int _actionType)
{
	if(_actionType == QuaternionLerp::GET_CONTROL_QUATERNIONS_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_CONTROL_QUATERNIONS");

		GetControlQuaternionsAction* derived = dynamic_cast<GetControlQuaternionsAction*>(action);
...
void ActionCreator::GET_CONTROL_QUATERNIONS_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_CONTROL_QUATERNIONS")
	{
		action = Trigger::_CreateAction(QuaternionLerp::GET_CONTROL_QUATERNIONS_MESSAGE);
