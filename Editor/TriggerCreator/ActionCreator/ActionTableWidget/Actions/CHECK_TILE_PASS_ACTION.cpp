...
void ActionTableWidget::CHECK_TILE_PASS_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "CHECK_TILE_PASS")
	{
...
bool ActionTableWidget::CHECK_TILE_PASS_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHECK_TILE_PASS") 
	{
...
QString ActionTableWidget::CHECK_TILE_PASS_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_TILE_PASS")
	{ 
		QString name;
...
bool ActionTableWidget::CHECK_TILE_PASS_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_TILE_PASS")
	{ 
...
bool ActionTableWidget::CHECK_TILE_PASS_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_TILE_PASS")
	{
...
bool ActionTableWidget::CHECK_TILE_PASS_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_TILE_PASS")
	{ 
...
void ActionTableWidget::CHECK_TILE_PASS_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHECK_TILE_PASS")
	{
...
void ActionCreator::CHECK_TILE_PASS_SetAction(int _actionType)
{
	if(_actionType == TiledSpace3D::CHECK_TILE_PASS_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "CHECK_TILE_PASS");

		CheckTilePassAction* derived = dynamic_cast<CheckTilePassAction*>(action);
...
void ActionCreator::CHECK_TILE_PASS_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "CHECK_TILE_PASS")
	{
		action = Trigger::_CreateAction(TiledSpace3D::CHECK_TILE_PASS_MESSAGE);
