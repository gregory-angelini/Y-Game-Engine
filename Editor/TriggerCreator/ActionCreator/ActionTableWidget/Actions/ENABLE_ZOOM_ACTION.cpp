...
void ActionTableWidget::ENABLE_ZOOM_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ENABLE_ZOOM")
	{
...
bool ActionTableWidget::ENABLE_ZOOM_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ENABLE_ZOOM") 
	{
...
QString ActionTableWidget::ENABLE_ZOOM_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_ZOOM")
	{ 
		QString name;
...
bool ActionTableWidget::ENABLE_ZOOM_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_ZOOM")
	{ 
...
bool ActionTableWidget::ENABLE_ZOOM_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_ZOOM")
	{
...
void ActionTableWidget::ENABLE_ZOOM_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_ZOOM")
	{
...
void ActionCreator::ENABLE_ZOOM_SetAction(int _actionType)
{
	if(_actionType == Camera3D::ENABLE_ZOOM_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ENABLE_ZOOM");

		EnableZoomAction* derived = dynamic_cast<EnableZoomAction*>(action);
...
void ActionCreator::ENABLE_ZOOM_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ENABLE_ZOOM")
	{
		action = Trigger::_CreateAction(Camera3D::ENABLE_ZOOM_MESSAGE);
