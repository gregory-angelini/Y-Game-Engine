...
void ActionTableWidget::ENABLE_COLLISION_POLYGONS_INFO_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ENABLE_COLLISION_POLYGONS_INFO")
	{
...
bool ActionTableWidget::ENABLE_COLLISION_POLYGONS_INFO_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ENABLE_COLLISION_POLYGONS_INFO") 
	{
...
QString ActionTableWidget::ENABLE_COLLISION_POLYGONS_INFO_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_COLLISION_POLYGONS_INFO")
	{ 
		QString name;
...
bool ActionTableWidget::ENABLE_COLLISION_POLYGONS_INFO_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_COLLISION_POLYGONS_INFO")
	{ 
...
bool ActionTableWidget::ENABLE_COLLISION_POLYGONS_INFO_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_COLLISION_POLYGONS_INFO")
	{
...
void ActionTableWidget::ENABLE_COLLISION_POLYGONS_INFO_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_COLLISION_POLYGONS_INFO")
	{
...
void ActionCreator::ENABLE_COLLISION_POLYGONS_INFO_SetAction(int _actionType)
{
	if(_actionType == PolygonalSurface::ENABLE_COLLISION_POLYGONS_INFO_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ENABLE_COLLISION_POLYGONS_INFO");

		EnableCollisionPolygonsInfoAction* derived = dynamic_cast<EnableCollisionPolygonsInfoAction*>(action);
...
void ActionCreator::ENABLE_COLLISION_POLYGONS_INFO_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ENABLE_COLLISION_POLYGONS_INFO")
	{
		action = Trigger::_CreateAction(PolygonalSurface::ENABLE_COLLISION_POLYGONS_INFO_MESSAGE);
