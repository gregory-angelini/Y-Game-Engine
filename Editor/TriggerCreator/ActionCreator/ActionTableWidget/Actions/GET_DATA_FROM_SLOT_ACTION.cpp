...
void ActionTableWidget::GET_DATA_FROM_SLOT_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "GET_DATA_FROM_SLOT")
	{
		if(_index.column() == VALUE)
		{
			switch(_index.row())
			{
				case ARG1_PROPERTY:
				{
					...
					{
						_editor = UniversalDelegate::ABSTRACT_VARIABLE_EDITOR;
						return;
					}
...
bool ActionTableWidget::GET_DATA_FROM_SLOT_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "GET_DATA_FROM_SLOT") 
	{
		...
		else if(_index.row() == ARG1_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(ARG1_PROPERTY, DATA_TYPE, "Abstract variable"); 
		}
		...
		{ 
			...
			if(item(ARG2_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG2_PROPERTY, CONTENT, "Slot index"); 
...
QString ActionTableWidget::GET_DATA_FROM_SLOT_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_DATA_FROM_SLOT")
	{ 
		QString name;
...
bool ActionTableWidget::GET_DATA_FROM_SLOT_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_DATA_FROM_SLOT")
	{ 
...
bool ActionTableWidget::GET_DATA_FROM_SLOT_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_DATA_FROM_SLOT")
	{
		UpdateAbstractVariable(ARG1_PROPERTY);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::GET_DATA_FROM_SLOT_UpdateArg2VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_DATA_FROM_SLOT")
	{
...
void ActionTableWidget::GET_DATA_FROM_SLOT_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "GET_DATA_FROM_SLOT")
	{
		if(item(_index.row(), ARG_MODE)->text() == "VARIABLE")
		{
			...
			{
				AbstractVariableDropEvent(_it, _index, _suffix, _fileName);
			}
...
void ActionCreator::GET_DATA_FROM_SLOT_SetAction(int _actionType)
{
	if(_actionType == Set::GET_DATA_FROM_SLOT_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "GET_DATA_FROM_SLOT");

		GetDataFromSlotAction* derived = dynamic_cast<GetDataFromSlotAction*>(action);
...
void ActionCreator::GET_DATA_FROM_SLOT_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "GET_DATA_FROM_SLOT")
	{
		action = Trigger::_CreateAction(Set::GET_DATA_FROM_SLOT_MESSAGE);
