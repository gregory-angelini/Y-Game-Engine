...
void ActionTableWidget::DISPLACE_ROTATION_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "DISPLACE_ROTATION")
	{
...
bool ActionTableWidget::DISPLACE_ROTATION_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "DISPLACE_ROTATION") 
	{
		...
		{ 
			...
			if(item(ARG1_PROPERTY, ARG_MODE)->text() == "KEY")
			{
				SetCellText(ARG1_PROPERTY, CONTENT, "Offset"); 
...
QString ActionTableWidget::DISPLACE_ROTATION_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "DISPLACE_ROTATION")
	{ 
		QString name;
...
bool ActionTableWidget::DISPLACE_ROTATION_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "DISPLACE_ROTATION")
	{ 
...
bool ActionTableWidget::DISPLACE_ROTATION_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "DISPLACE_ROTATION")
	{
...
void ActionTableWidget::DISPLACE_ROTATION_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "DISPLACE_ROTATION")
	{
...
void ActionCreator::DISPLACE_ROTATION_SetAction(int _actionType)
{
	if(_actionType == RotatableObject::DISPLACE_ROTATION_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "DISPLACE_ROTATION");

		DisplaceRotationAction* derived = dynamic_cast<DisplaceRotationAction*>(action);
...
void ActionCreator::DISPLACE_ROTATION_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "DISPLACE_ROTATION")
	{
		action = Trigger::_CreateAction(RotatableObject::DISPLACE_ROTATION_MESSAGE);
