...
void ActionTableWidget::ENABLE_SHOW_GetEditorTypeForCell(const QModelIndex& _index, const QString& _actionType, int& _editor)
{
	if(_actionType == "ENABLE_SHOW")
	{
...
bool ActionTableWidget::ENABLE_SHOW_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ENABLE_SHOW") 
	{
		...
		else if(_index.row() == TARGET_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(TARGET_PROPERTY, DATA_TYPE, "Graphics object"); 
		}
		return true;
	}  
	return false;
...
QString ActionTableWidget::ENABLE_SHOW_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_SHOW")
	{ 
		QString name;
...
bool ActionTableWidget::ENABLE_SHOW_UpdateTargetVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_SHOW")
	{ 
		UpdateGraphicsObject(TARGET_PROPERTY);
		return true;
	}
	return false;
}
...
bool ActionTableWidget::ENABLE_SHOW_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_SHOW")
	{
...
void ActionTableWidget::ENABLE_SHOW_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_SHOW")
	{
...
void ActionCreator::ENABLE_SHOW_SetAction(int _actionType)
{
	if(_actionType == GraphicsObject::ENABLE_SHOW_MESSAGE)
	{
		actionTable->SetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE, "ENABLE_SHOW");

		EnableShowAction* derived = dynamic_cast<EnableShowAction*>(action);
...
void ActionCreator::ENABLE_SHOW_Create(void)
{
	if(actionTable->GetCellText(ActionTableWidget::TYPE_PROPERTY, ActionTableWidget::VALUE) == "ENABLE_SHOW")
	{
		action = Trigger::_CreateAction(GraphicsObject::ENABLE_SHOW_MESSAGE);
