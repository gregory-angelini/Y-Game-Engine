...
void EventTableWidget::CHANGE_FRONT_TEXTURED_ANIMATION_GetEditorTypeForCell(const QModelIndex& _index, const QString& _eventType, int& _editor)
{
	if(_eventType == "CHANGE_FRONT_TEXTURED_ANIMATION")
	{
...
bool EventTableWidget::CHANGE_FRONT_TEXTURED_ANIMATION_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_FRONT_TEXTURED_ANIMATION") 
	{
		...
		else if(_index.row() == ARG1_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(ARG1_PROPERTY, DATA_TYPE, "Textured animation"); 
		}
		else if(_index.row() == SOURCE_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(SOURCE_PROPERTY, DATA_TYPE, "Texturing surface"); 
		}
		return true;
	}  
	return false;
...
QString EventTableWidget::CHANGE_FRONT_TEXTURED_ANIMATION_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_FRONT_TEXTURED_ANIMATION")
	{ 
		QString name;
...
bool EventTableWidget::CHANGE_FRONT_TEXTURED_ANIMATION_UpdateSourceVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_FRONT_TEXTURED_ANIMATION")
	{ 
...
bool EventTableWidget::CHANGE_FRONT_TEXTURED_ANIMATION_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_FRONT_TEXTURED_ANIMATION")
	{
		UpdateTexturedAnimation(ARG1_PROPERTY);
		return true;
	}
	return false;
}
...
void EventTableWidget::CHANGE_FRONT_TEXTURED_ANIMATION_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_FRONT_TEXTURED_ANIMATION")
	{
...
void EventCreator::CHANGE_FRONT_TEXTURED_ANIMATION_SetEvent(int _eventType)
{
	if(_eventType == TexturingSurface::CHANGE_FRONT_TEXTURED_ANIMATION_MESSAGE)
	{
		eventTable->SetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE, "CHANGE_FRONT_TEXTURED_ANIMATION");

		ChangeFrontTexturedAnimationEvent* derived = dynamic_cast<ChangeFrontTexturedAnimationEvent*>(event);
...
void EventCreator::CHANGE_FRONT_TEXTURED_ANIMATION_Create(void)
{
	if(eventTable->GetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE) == "CHANGE_FRONT_TEXTURED_ANIMATION")
	{
		event = Trigger::_CreateEvent(TexturingSurface::CHANGE_FRONT_TEXTURED_ANIMATION_MESSAGE);
