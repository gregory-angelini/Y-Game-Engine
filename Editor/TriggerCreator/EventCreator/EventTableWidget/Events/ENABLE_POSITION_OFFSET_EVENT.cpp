...
void EventTableWidget::ENABLE_POSITION_OFFSET_GetEditorTypeForCell(const QModelIndex& _index, const QString& _eventType, int& _editor)
{
	if(_eventType == "ENABLE_POSITION_OFFSET")
	{
		if(_index.column() == VALUE)
		{
			switch(_index.row())
			{
				...
				case SOURCE_PROPERTY:
				{
					...
					{
						_editor = UniversalDelegate::TEXTURED_ANIMATION_EDITOR;
						return;
					}
...
bool EventTableWidget::ENABLE_POSITION_OFFSET_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "ENABLE_POSITION_OFFSET") 
	{
		...
		else if(_index.row() == SOURCE_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(SOURCE_PROPERTY, DATA_TYPE, "Textured animation"); 
		}
		return true;
	}  
	return false;
...
QString EventTableWidget::ENABLE_POSITION_OFFSET_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_POSITION_OFFSET")
	{ 
		QString name;
...
bool EventTableWidget::ENABLE_POSITION_OFFSET_UpdateSourceVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_POSITION_OFFSET")
	{ 
		UpdateTexturedAnimation(SOURCE_PROPERTY);
		return true;
	}
	return false;
}
...
bool EventTableWidget::ENABLE_POSITION_OFFSET_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_POSITION_OFFSET")
	{
...
void EventTableWidget::ENABLE_POSITION_OFFSET_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "ENABLE_POSITION_OFFSET")
	{
		if(item(_index.row(), ARG_MODE)->text() == "VARIABLE")
		{
			...
			{
				TexturedAnimationDropEvent(_it, _index, _suffix, _fileName);
			}
		}
...
void EventCreator::ENABLE_POSITION_OFFSET_SetEvent(int _eventType)
{
	if(_eventType == TexturedAnimation::ENABLE_POSITION_OFFSET_MESSAGE)
	{
		eventTable->SetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE, "ENABLE_POSITION_OFFSET");

		EnablePositionOffsetEvent* derived = dynamic_cast<EnablePositionOffsetEvent*>(event);
		...
		if(derived)
		{
			if(derived->IsArgsEnabled())
			{
				if(derived->GetArgState(AbstractEvent::KEY, AbstractEvent::ARG1))
				{
					...
					eventTable->SetCellText(EventTableWidget::ARG1_PROPERTY, EventTableWidget::VALUE, derived->GetBooleanKeyArg(AbstractEvent::ARG1) ? "true" : "false");
				}
...
void EventCreator::ENABLE_POSITION_OFFSET_Create(void)
{
	if(eventTable->GetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE) == "ENABLE_POSITION_OFFSET")
	{
		event = Trigger::_CreateEvent(TexturedAnimation::ENABLE_POSITION_OFFSET_MESSAGE);
		...
		if(event)
		{
			...
			else
			{
				...
				{
					event->SetBooleanKeyArg(AbstractEvent::ARG1, eventTable->GetCellText(EventTableWidget::ARG1_PROPERTY, EventTableWidget::VALUE) == "true" ? true : false);
				}
