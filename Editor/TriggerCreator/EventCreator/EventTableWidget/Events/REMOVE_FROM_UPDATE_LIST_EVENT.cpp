...
void EventTableWidget::REMOVE_FROM_UPDATE_LIST_GetEditorTypeForCell(const QModelIndex& _index, const QString& _eventType, int& _editor)
{
	if(_eventType == "REMOVE_FROM_UPDATE_LIST")
	{
...
bool EventTableWidget::REMOVE_FROM_UPDATE_LIST_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "REMOVE_FROM_UPDATE_LIST") 
	{
		...
		else if(_index.row() == SOURCE_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(SOURCE_PROPERTY, DATA_TYPE, "Logic scene"); 
		}
		return true;
	}  
	return false;
...
QString EventTableWidget::REMOVE_FROM_UPDATE_LIST_CreateAutoName(void)
{
...
bool EventTableWidget::REMOVE_FROM_UPDATE_LIST_UpdateSourceVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REMOVE_FROM_UPDATE_LIST")
	{ 
...
bool EventTableWidget::REMOVE_FROM_UPDATE_LIST_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REMOVE_FROM_UPDATE_LIST")
	{
...
void EventTableWidget::REMOVE_FROM_UPDATE_LIST_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "REMOVE_FROM_UPDATE_LIST")
	{
...
void EventCreator::REMOVE_FROM_UPDATE_LIST_SetEvent(int _eventType)
{
	if(_eventType == LogicScene::REMOVE_FROM_UPDATE_LIST_MESSAGE)
	{
		eventTable->SetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE, "REMOVE_FROM_UPDATE_LIST");

		RemoveFromUpdateListEvent* derived = dynamic_cast<RemoveFromUpdateListEvent*>(event);
...
void EventCreator::REMOVE_FROM_UPDATE_LIST_Create(void)
{
	if(eventTable->GetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE) == "REMOVE_FROM_UPDATE_LIST")
	{
		event = Trigger::_CreateEvent(LogicScene::REMOVE_FROM_UPDATE_LIST_MESSAGE);
