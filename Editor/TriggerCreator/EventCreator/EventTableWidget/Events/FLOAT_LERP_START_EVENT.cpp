...
void EventTableWidget::FLOAT_LERP_START_GetEditorTypeForCell(const QModelIndex& _index, const QString& _eventType, int& _editor)
{
	if(_eventType == "FLOAT_LERP_START")
	{
...
bool EventTableWidget::FLOAT_LERP_START_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "FLOAT_LERP_START") 
	{
...
QString EventTableWidget::FLOAT_LERP_START_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "FLOAT_LERP_START")
	{ 
		QString name;
...
bool EventTableWidget::FLOAT_LERP_START_UpdateSourceVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "FLOAT_LERP_START")
	{ 
...
void EventTableWidget::FLOAT_LERP_START_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "FLOAT_LERP_START")
	{
...
void EventCreator::FLOAT_LERP_START_SetEvent(int _eventType)
{
	if(_eventType == FloatLerp::FLOAT_LERP_START_MESSAGE)
	{
		eventTable->SetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE, "FLOAT_LERP_START");

		FloatLerpStartEvent* derived = dynamic_cast<FloatLerpStartEvent*>(event);
...
void EventCreator::FLOAT_LERP_START_Create(void)
{
	if(eventTable->GetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE) == "FLOAT_LERP_START")
	{
		event = Trigger::_CreateEvent(FloatLerp::FLOAT_LERP_START_MESSAGE);
