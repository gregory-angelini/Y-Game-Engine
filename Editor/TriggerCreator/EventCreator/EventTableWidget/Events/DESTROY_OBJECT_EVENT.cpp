...
void EventTableWidget::DESTROY_OBJECT_GetEditorTypeForCell(const QModelIndex& _index, const QString& _eventType, int& _editor)
{
	if(_eventType == "DESTROY_OBJECT")
	{
...
bool EventTableWidget::DESTROY_OBJECT_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "DESTROY_OBJECT") 
	{
...
QString EventTableWidget::DESTROY_OBJECT_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "DESTROY_OBJECT")
	{ 
		QString name;
...
bool EventTableWidget::DESTROY_OBJECT_UpdateSourceVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "DESTROY_OBJECT")
	{ 
...
void EventTableWidget::DESTROY_OBJECT_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "DESTROY_OBJECT")
	{
...
void EventCreator::DESTROY_OBJECT_SetEvent(int _eventType)
{
	if(_eventType == AbstractObject::DESTROY_OBJECT_MESSAGE)
	{
		eventTable->SetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE, "DESTROY_OBJECT");

		DestroyObjectEvent* derived = dynamic_cast<DestroyObjectEvent*>(event);
...
void EventCreator::DESTROY_OBJECT_Create(void)
{
	if(eventTable->GetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE) == "DESTROY_OBJECT")
	{
		event = Trigger::_CreateEvent(AbstractObject::DESTROY_OBJECT_MESSAGE);
