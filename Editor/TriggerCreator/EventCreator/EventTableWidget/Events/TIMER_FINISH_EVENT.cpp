...
void EventTableWidget::TIMER_FINISH_GetEditorTypeForCell(const QModelIndex& _index, const QString& _eventType, int& _editor)
{
	if(_eventType == "TIMER_FINISH")
	{
		if(_index.column() == VALUE)
		{
			switch(_index.row())
			{
				case SOURCE_PROPERTY:
				{
					...
					{
						_editor = UniversalDelegate::TIMER_EDITOR;
						return;
					}
...
bool EventTableWidget::TIMER_FINISH_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "TIMER_FINISH") 
	{
		...
		if(_index.row() == SOURCE_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(SOURCE_PROPERTY, DATA_TYPE, "Timer"); 
		}
		return true;
	}  
	return false;
...
QString EventTableWidget::TIMER_FINISH_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "TIMER_FINISH")
	{ 
		QString name;
...
bool EventTableWidget::TIMER_FINISH_UpdateSourceVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "TIMER_FINISH")
	{ 
		UpdateTimer(SOURCE_PROPERTY);
		return true;
	}
	return false;
}
...
void EventTableWidget::TIMER_FINISH_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "TIMER_FINISH")
	{
		if(item(_index.row(), ARG_MODE)->text() == "VARIABLE")
		{
			...
			{
				TimerDropEvent(_it, _index, _suffix, _fileName);
			}
		}
...
void EventCreator::TIMER_FINISH_SetEvent(int _eventType)
{
	if(_eventType == Timer::TIMER_FINISH_MESSAGE)
	{
		eventTable->SetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE, "TIMER_FINISH");

		TimerFinishEvent* derived = dynamic_cast<TimerFinishEvent*>(event);
...
void EventCreator::TIMER_FINISH_Create(void)
{
	if(eventTable->GetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE) == "TIMER_FINISH")
	{
		event = Trigger::_CreateEvent(Timer::TIMER_FINISH_MESSAGE);
