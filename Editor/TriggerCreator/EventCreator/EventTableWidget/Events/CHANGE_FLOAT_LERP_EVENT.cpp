...
void EventTableWidget::CHANGE_FLOAT_LERP_GetEditorTypeForCell(const QModelIndex& _index, const QString& _eventType, int& _editor)
{
	if(_eventType == "CHANGE_FLOAT_LERP")
	{
...
bool EventTableWidget::CHANGE_FLOAT_LERP_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "CHANGE_FLOAT_LERP") 
	{
		...
		else if(_index.row() == SOURCE_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(SOURCE_PROPERTY, DATA_TYPE, "Lerping object"); 
		}
		return true;
	}  
	return false;
...
QString EventTableWidget::CHANGE_FLOAT_LERP_CreateAutoName(void)
{
...
bool EventTableWidget::CHANGE_FLOAT_LERP_UpdateSourceVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_FLOAT_LERP")
	{ 
		UpdateLerpingObject(SOURCE_PROPERTY);
		return true;
	}
	return false;
}
...
bool EventTableWidget::CHANGE_FLOAT_LERP_UpdateArg1VariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_FLOAT_LERP")
	{
...
void EventTableWidget::CHANGE_FLOAT_LERP_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "CHANGE_FLOAT_LERP")
	{
...
void EventCreator::CHANGE_FLOAT_LERP_SetEvent(int _eventType)
{
	if(_eventType == LerpingObject::CHANGE_FLOAT_LERP_MESSAGE)
	{
		eventTable->SetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE, "CHANGE_FLOAT_LERP");

		ChangeFloatLerpEvent* derived = dynamic_cast<ChangeFloatLerpEvent*>(event);
...
void EventCreator::CHANGE_FLOAT_LERP_Create(void)
{
	if(eventTable->GetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE) == "CHANGE_FLOAT_LERP")
	{
		event = Trigger::_CreateEvent(LerpingObject::CHANGE_FLOAT_LERP_MESSAGE);
