...
void EventTableWidget::TRIGGER_FINISH_GetEditorTypeForCell(const QModelIndex& _index, const QString& _eventType, int& _editor)
{
	if(_eventType == "TRIGGER_FINISH")
	{
		if(_index.column() == VALUE)
		{
			switch(_index.row())
			{
				case SOURCE_PROPERTY:
				{
					...
					{
						_editor = UniversalDelegate::TRIGGER_EDITOR;
						return;
					}
...
bool EventTableWidget::TRIGGER_FINISH_UpdateRow(const QModelIndex& _index, const QString& _type)
{
	if(_type == "TRIGGER_FINISH") 
	{
		...
		if(_index.row() == SOURCE_PROPERTY && _index.column() == ARG_MODE)
		{
			...
			SetCellText(SOURCE_PROPERTY, DATA_TYPE, "Trigger"); 
		}
		return true;
	}  
	return false;
...
QString EventTableWidget::TRIGGER_FINISH_CreateAutoName(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "TRIGGER_FINISH")
	{ 
		QString name;
...
bool EventTableWidget::TRIGGER_FINISH_UpdateSourceVariablePropertyStatus(void)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "TRIGGER_FINISH")
	{ 
		UpdateTrigger(SOURCE_PROPERTY);
		return true;
	}
	return false;
}
...
void EventTableWidget::TRIGGER_FINISH_DropEvent(QTableWidgetItem* _it, const QModelIndex& _index, const QString& _suffix, QString& _fileName)
{
	if(item(TYPE_PROPERTY, VALUE)->text() == "TRIGGER_FINISH")
	{
		if(item(_index.row(), ARG_MODE)->text() == "VARIABLE")
		{
			...
			{
				TriggerDropEvent(_it, _index, _suffix, _fileName);
			}
		}
...
void EventCreator::TRIGGER_FINISH_SetEvent(int _eventType)
{
	if(_eventType == Trigger::TRIGGER_FINISH_MESSAGE)
	{
		eventTable->SetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE, "TRIGGER_FINISH");

		TriggerFinishEvent* derived = dynamic_cast<TriggerFinishEvent*>(event);
...
void EventCreator::TRIGGER_FINISH_Create(void)
{
	if(eventTable->GetCellText(EventTableWidget::TYPE_PROPERTY, EventTableWidget::VALUE) == "TRIGGER_FINISH")
	{
		event = Trigger::_CreateEvent(Trigger::TRIGGER_FINISH_MESSAGE);
