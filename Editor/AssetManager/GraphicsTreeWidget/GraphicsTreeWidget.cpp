...
GraphicsTreeWidget::GraphicsTreeWidget(QWidget* _parent): QTreeWidget(_parent), root(NIL), scrollTimerId(0), ignoreSelection(false), dropItem(NIL), autoParentEnable(true)
{
	...
	AssetLibrary::_GetAssets().Connect(AssetLibrary::CREATE_OBJECT_MESSAGE, this, Caller<>(this, &GraphicsTreeWidget::ObjectIsCreated));
...
GraphicsTreeWidget::~GraphicsTreeWidget(void)
{}




void GraphicsTreeWidget::CreateContextMenu(void)
{
...
void GraphicsTreeWidget::RemoveFromSceneSelectedItems(void)
{
	QList<QTreeWidgetItem*> list = selectedItems();
	for(int i = 0; i < list.size(); i++) 
	{
		GraphicsSceneItem* item = dynamic_cast<GraphicsSceneItem*>(list.at(i));
		QTreeWidgetItem* parent = item->parent();

		if(parent)
		{	
			GraphicsObject* object = dynamic_cast<GraphicsSceneItem*>(item)->GetObjectPtr();
						
			GraphicsScene* scene = dynamic_cast<GraphicsScene*>(dynamic_cast<GraphicsSceneItem*>(parent)->GetObjectPtr());

			if(scene)
			{
				scene->GetRenderList().Remove(object);
			}
		}
	}
}
...
void GraphicsTreeWidget::PickCurrentItem(void)
{
	GraphicsSceneItem* it = static_cast<GraphicsSceneItem*>(itemAt(cursorPoint));
...
void GraphicsTreeWidget::SaveToFileSelectedItems(void)
{
	QList<QTreeWidgetItem*> list = selectedItems();
	for(int i = 0; i < list.size(); i++) 
	{
...
void GraphicsTreeWidget::CopyCurrentItem(void)
{
	GraphicsSceneItem* it = static_cast<GraphicsSceneItem*>(itemAt(cursorPoint));
	...
	if(it)
	{
		...
		{
			if(it->type() == GraphicsSceneItem::OBJECT)
			{
...
void GraphicsTreeWidget::RenameCurrentItem(void)
{
	GraphicsSceneItem* it = static_cast<GraphicsSceneItem*>(itemAt(cursorPoint));
...
void GraphicsTreeWidget::DestroySelectedItems(void)
{
	...
	{ 
		...
		if(it)
		{
			GraphicsSceneItem* item = dynamic_cast<GraphicsSceneItem*>(it); 
...
GraphicsSceneItem* GraphicsTreeWidget::FindLeaf(QTreeWidgetItem* _item)
{
	GraphicsSceneItem* leaf = NIL;

	if(_item)
	{
		leaf = dynamic_cast<GraphicsSceneItem*>(_item->child(0));

		if(leaf)
		{
			while(leaf->child(0))
			{
				leaf = dynamic_cast<GraphicsSceneItem*>(leaf->child(0));
			}
		}
	}
	return leaf;
...
QTreeWidgetItem* GraphicsTreeWidget::FindLeaf(QList<QTreeWidgetItem*>& _list)
{ 
...
void GraphicsTreeWidget::NewAsset(void)
{
...
void GraphicsTreeWidget::mouseReleaseEvent(QMouseEvent* _event)
{
...
void GraphicsTreeWidget::mouseDoubleClickEvent(QMouseEvent* _event)
{
	{
		{
		} 
...
void GraphicsTreeWidget::mouseMoveEvent(QMouseEvent* _event)
{
	cursorPoint = _event->pos();
...
void GraphicsTreeWidget::mousePressEvent(QMouseEvent* _event)
{
	cursorPoint = _event->pos();
...
void GraphicsTreeWidget::UpdateContextMenu(void)
{
	...
	if(it)
	{
		...
		{
			if(it->type() == GraphicsSceneItem::SCENE)
			{
				contextMenu->addAction(importItemAct); 
			}
			...
			GraphicsSceneItem* object = dynamic_cast<GraphicsSceneItem*>(it);
			...
			if(object)
			{
				...
				if(object->GetObjectPtr()->IsClassPropertyExist(Scene::SCENE_CLASS))
				{
...
void GraphicsTreeWidget::SaveAllAssets(void)
{
	SaveAllAssets(root);
}
...
void GraphicsTreeWidget::SaveAllAssets(QTreeWidgetItem* _it)
{
	if(_it)
	{
		if(_it != root)
		{
			GraphicsSceneItem* item = dynamic_cast<GraphicsSceneItem*>(_it);

			item->GetObjectPtr()->SaveToFile();
		}
		
		GraphicsSceneItem* search;
		int child = 0;
...
void GraphicsTreeWidget::SaveLayersStructure(void)
{
...
void GraphicsTreeWidget::SaveLayersStructure(QTreeWidgetItem* _it)
{
	if(_it)
	{
		if(_it != root)
		{
			if(_it->type() == GraphicsSceneItem::SCENE)
			{
				GraphicsSceneItem* item = dynamic_cast<GraphicsSceneItem*>(_it);
...
void GraphicsTreeWidget::keyPressEvent(QKeyEvent* _event)
{
...
void GraphicsTreeWidget::keyReleaseEvent(QKeyEvent* _event)
{  
...
QPoint GraphicsTreeWidget::GetCursorPoint(void)const
{
	return cursorPoint;
}
...
void GraphicsTreeWidget::ObjectIsCreated(AbstractObject* _object)
{
	if(FindItem(_object->GetName().c_str()) == NIL)
	{
		if(_object->IsClassPropertyExist(GraphicsObject::GRAPHICS_OBJECT_CLASS))
		{
			GraphicsSceneItem* parent = NIL;
			GraphicsSceneItem::Type assetType = GetAssetType(_object);
			GraphicsSceneItem* item = AddAsset(_object->GetName().c_str(), assetType, _object->GetClassProperties(), dynamic_cast<GraphicsObject*>(_object), parent);
		
			if(autoParentEnable)
			{
				parent = dynamic_cast<GraphicsSceneItem*>(currentItem());

				if(parent && parent != root)
				{
					if(parent->GetObjectPtr()->IsClassPropertyExist(GraphicsScene::GRAPHICS_SCENE_CLASS))
					{
						GraphicsScene* scene = dynamic_cast<GraphicsScene*>(dynamic_cast<GraphicsSceneItem*>(parent)->GetObjectPtr());

						if(scene)
						{
							scene->AddToRenderList(item->GetObjectPtr());
						}
					}
				}
			}

			_object->Connect(AbstractObject::DESTROY_OBJECT_MESSAGE, this, Caller<>(this, &GraphicsTreeWidget::ObjectIsRemoved));
			_object->Connect(AbstractObject::CHANGE_NAME_MESSAGE, this, Caller<>(this, &GraphicsTreeWidget::ObjectIsRenamed));
		}
	}
}

GraphicsSceneItem::Type GraphicsTreeWidget::GetAssetType(AbstractObject* _object)
{
	if(_object)
	{
		if(_object->IsClassPropertyExist(GraphicsScene::GRAPHICS_SCENE_CLASS))
		{
			return GraphicsSceneItem::SCENE;
		}
		else if(_object->IsClassPropertyExist(TiledSpace3D::TILED_SPACE_3D_CLASS))
		{
			return GraphicsSceneItem::TILED_SPACE;
		}
	}

	return GraphicsSceneItem::OBJECT; 
}




void GraphicsTreeWidget::ReviewAsset(void)
{
	GraphicsSceneItem* it = static_cast<GraphicsSceneItem*>(itemAt(cursorPoint));
...
void GraphicsTreeWidget::ImportAsset(void)
{
...
void GraphicsTreeWidget::ExpandRoot(void)
{
	expandItem(root);
}


void GraphicsTreeWidget::CreateRoot(void)
{
	GraphicsSceneItem* it = new GraphicsSceneItem(invisibleRootItem(), GraphicsSceneItem::SCENE, 0);
	it->treeWidget = this;

	it->setFlags(Qt::ItemIsEnabled | Qt::ItemIsDropEnabled);

	it->setIcon(ICON_PROPERTY, GetIcon(GraphicsSceneItem::SCENE));
	...
	it->setText(TYPE_PROPERTY, GetStringOfAssetType(GraphicsSceneItem::SCENE));
...
GraphicsSceneItem* GraphicsTreeWidget::AddAsset(QString _name, GraphicsSceneItem::Type _assetType, int64 _objectClass, GraphicsObject* _object, GraphicsSceneItem* _parent)
{
	GraphicsSceneItem* item = FindItem(_name);

	if(item == NIL)
	{
		if(_parent == NIL)
		{
			item = new GraphicsSceneItem(root, _assetType, _objectClass);
		}
		else 
		{
			item = new GraphicsSceneItem(root, _assetType, _objectClass);
		...
		if(_assetType == GraphicsSceneItem::SCENE)
		{
			item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable | Qt::ItemIsDragEnabled | Qt::ItemIsDropEnabled);

			_object->Connect(GraphicsScene::ADD_TO_RENDER_LIST_MESSAGE, item, Caller<>(item, &GraphicsSceneItem::AddToGraphicsScene));
			_object->Connect(GraphicsScene::REMOVE_FROM_RENDER_LIST_MESSAGE, item, Caller<>(item, &GraphicsSceneItem::RemoveFromGraphicsScene));
		}
		else
		{
			item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable | Qt::ItemIsDragEnabled | Qt::ItemIsDropEnabled);
		...
		{ 
			...
			if(_object->IsClassPropertyExist(GraphicsScene::GRAPHICS_SCENE_CLASS))
			{
				GraphicsScene* graphicsScene = dynamic_cast<GraphicsScene*>(_object);

				if(graphicsScene->GetRenderList().IsNotEmpty())
				{ 
					autoParentEnable = false;
					MagicList<GraphicsObject> objectList = graphicsScene->GetRenderList();

					for(MagicList<GraphicsObject>::AddressDerived* it = objectList.GetHeadAddress(); it != NIL; it = it->GetRight())
					{
						item->AddToGraphicsScene(it->GetNode());
					}
...
GraphicsSceneItem* GraphicsTreeWidget::FindItem(QString _name, QString _parent)
{
...
GraphicsSceneItem* GraphicsTreeWidget::FindItem(QTreeWidgetItem* _it, QString _name, QString _parent)
{
	if(_it)
	{
		if(_it != root)
...
GraphicsSceneItem* GraphicsTreeWidget::FindItem(QString _name)
{
...
GraphicsSceneItem* GraphicsTreeWidget::FindItem(QTreeWidgetItem* _it, QString _name)
{
	if(_it)
	{
		if(_it != root)
...
QString GraphicsTreeWidget::GetStringOfAssetType(GraphicsSceneItem::Type _assetType)const
{
	switch(_assetType)
	{
		case GraphicsSceneItem::OBJECT:
		{
			return "OBJECT";
		}

		case GraphicsSceneItem::SCENE:
		{
			return "SCENE";
		}
		
		case GraphicsSceneItem::TILED_SPACE:
		{
			return "SCENE";
		}
	}
...
QPixmap GraphicsTreeWidget::GetIcon(int _assetType)
{
	switch(_assetType)
	{
		case GraphicsSceneItem::SCENE:
...
void GraphicsTreeWidget::ObjectIsRemoved(AbstractObject* _object)
{
	GraphicsSceneItem* item;
...
void GraphicsTreeWidget::ObjectIsRenamed(AbstractObject::NameInfo* _info)
{
	GraphicsSceneItem* it;
...
void GraphicsTreeWidget::timerEvent(QTimerEvent* _event) 
{
...
void GraphicsTreeWidget::Scroll(void) 
{
...
void GraphicsTreeWidget::dragMoveEvent(QDragMoveEvent* _event)
{
	...
	if(it)
	{
		if(it->type() == GraphicsSceneItem::SCENE && dropIndicatorPos == QAbstractItemView::OnItem)
		{
...
void GraphicsTreeWidget::dropEvent(QDropEvent* _event)
{
	...
	if(_event->dropAction() != Qt::IgnoreAction)
	{
		QModelIndex index = indexAt(_event->pos());
		QTreeWidgetItem* it = itemFromIndex(index);
		if(it && it->type() == GraphicsSceneItem::SCENE)
		{	
			...
			for(int i = 0; i < selectedList.size(); ++i)
			{
				if(it == root)
				{
					...
					if(parent)
					{
						GraphicsObject* object = dynamic_cast<GraphicsSceneItem*>(selectedList.at(i))->GetObjectPtr();
				...
				else
				{
					...
					GraphicsScene* scene = dynamic_cast<GraphicsScene*>(dynamic_cast<GraphicsSceneItem*>(it)->GetObjectPtr());
...
void GraphicsTreeWidget::dragEnterEvent(QDragEnterEvent* _event)
{
