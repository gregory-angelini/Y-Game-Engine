#include "PlayingObjectEditor.h"
#include <Editor/Editor/Editor.h>
#include <Engine/Core/Time/Timer/Timer.h>



PlayingObjectEditor::PlayingObjectEditor(QWidget* _parent): QWidget(_parent)
{
...
void PlayingObjectEditor::FocusOut(void)
{
	emit FocusOut(this);
}


void PlayingObjectEditor::SetFileName(QString _fileName)
{
	fileNameEdit->setText(_fileName);
}


QString PlayingObjectEditor::GetFileName(void)const
{
	return fileNameEdit->text();
}


void PlayingObjectEditor::NameIsChanged(QString _name)
{
	...
	else
	{
		...
		bool exist = false;
		if(AssetLibrary::_IsAssetExist(_name.toStdString(), AssetLibrary::TIMER_ASSET))
		{
			exist = true;
...
void PlayingObjectEditor::OpenFinder(void)
{
	QFileDialog dialog(this, "Choose playing object");
	dialog.setFileMode(QFileDialog::ExistingFile);
	
	dialog.setNameFilter(QString(" *.") + Timer::_GetFileSuffix().c_str() +
...
void PlayingObjectEditor::CheckArg(void)
{
	...
	index = 0;
	bool exist = false;
	if(AssetLibrary::_IsAssetExist(fileNameEdit->text().toStdString(), AssetLibrary::TIMER_ASSET) ||
...
void PlayingObjectEditor::EditFloatLerp(void)
{
...
void PlayingObjectEditor::EditTexturedAnimation(void)
{
	StringANSI name = fileNameEdit->text().toStdString();
...
void PlayingObjectEditor::CreateObject(void)
{  
	StringANSI name = fileNameEdit->text().toStdString();
...
void PlayingObjectEditor::_SetLineEditTextFormat(QLineEdit* _lineEdit, const QList<QTextLayout::FormatRange>& _formats)
{
    if(_lineEdit)
	{
...
void PlayingObjectEditor::_ClearLineEditTextFormat(QLineEdit* _lineEdit)
{
...
void PlayingObjectEditor::resizeEvent(QResizeEvent* _event)
{
